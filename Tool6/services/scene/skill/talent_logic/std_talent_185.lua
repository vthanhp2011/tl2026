...
local std_talent_174 = class("std_talent_174", base)
local DI_DamagesByValue_T = require "scene.skill.impact_logic.std_impact_003"
local DAMAGE_TYPE_RATE = DI_DamagesByValue_T.enum_DAMAGE_TYPE_RATE

function std_talent_174:is_specific_skill(skill_id)
    return skill_id == 535
end

function std_talent_174:get_refix_value(talent, level)
    local params = talent.params[level]
    return params[1]
end

function std_talent_174:on_damage_target(talent, level, id, _, reciver, damages, skill_id)
