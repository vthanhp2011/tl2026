...
local std_talent_618 = class("std_talent_618", base)

function std_talent_618:is_specific_skill(skill_id)
    return skill_id == 760
end

function std_talent_618:get_refix_value(talent, level)
    local params = talent.params[level]
    return params[1]
end

function std_talent_618:on_damage_target(talent, level, id, sender, reciver, damages, skill_id, imp)
    if self:is_specific_skill(skill_id) then
        if imp and imp:is_critical_hit() then
            local odd = self:get_reifx_value(talent, level)
            local n = math.random(1, 100)
            if n <= odd then
                local target_skill_id = 768
