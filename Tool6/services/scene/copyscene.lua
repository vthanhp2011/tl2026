...
local SceneCore = require "scene.copyscenecore":getinstance()
...
skynet.start(function()
    skynet.dispatch("lua", function(_, source, command, ...)
        local args = { ... }
        local request_start = skynet.now()
        if command == "get_params" then
        ...
        local request_end = skynet.now()
        local diff = request_end - request_start
        if diff > 100 then
            skynet.logw("scene warning slow log ts =", diff, ", requst =", command, ", source =", source, ", sceneid =", SceneCore:get_id(), ", args =", table.tostr(args))
        end
    end)
