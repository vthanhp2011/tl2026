...
function compound:can_use_ability(human)
    ...
    if ability_opera.pres_id > 0 and ability_opera.pres_id <= #prescription_list then
        ...
        if script_id ~= define.INVAILD_ID then
            return scriptenginer:call(script_id, self.DEF_ABILITY_CHECK, human:get_obj_id(), ability_opera.pres_id)
        else
...
function compound:on_proc_over(human)
    ...
    local script_id = prescription["脚本ID"]
    if script_id > 0 then
        local res = scriptenginer:call(script_id, self.DEF_ABILITY_CHECK, human:get_obj_id(), ability_opera.pres_id)
        if res ~= define.OPERATE_RESULT.OR_OK then
            return res
        end
    ...
    if script_id ~= define.INVAILD_ID then
        return scriptenginer:call(script_id, self.DEF_ABILITY_PRODUCE, human:get_obj_id(), ability_opera.pres_id)
    else
