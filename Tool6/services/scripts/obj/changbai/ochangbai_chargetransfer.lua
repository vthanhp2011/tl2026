...
local ochangbai_chargetransfer = class("ochangbai_chargetransfer", script_base)
ochangbai_chargetransfer.script_id = 400962
ochangbai_chargetransfer.g_name = "车传仁"
ochangbai_chargetransfer.g_transfer_target = {
    ...
    [6] = {["x"] = 186, ["z"] = 43, ["scene_num"] = 28}
}

ochangbai_chargetransfer.g_transfer_cost = 5000
function ochangbai_chargetransfer:OnDefaultEvent(selfId, targetId)
    self:UpdateEventList(selfId, targetId)
end

function ochangbai_chargetransfer:UpdateEventList(selfId, targetId)
    ...
    self:AddNumText("南诏", 9, 6)
    self:EndEvent()
    self:DispatchEventList(selfId, targetId)
end

function ochangbai_chargetransfer:OnEventRequest(selfId, targetId, arg, index)
...
function ochangbai_chargetransfer:NotifyFailTips(selfId, targetId, Tip)
    self:BeginEvent(self.script_id)
    self:AddText(Tip)
    self:EndEvent()
    self:DispatchEventList(selfId, targetId)
...
return ochangbai_chargetransfer
