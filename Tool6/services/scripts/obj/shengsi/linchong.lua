...
local linchong = class("linchong", script_base)
linchong.script_id = 892019
linchong.g_MonsterId = 13558
linchong.g_CreateId = 13510
linchong.g_posX = 41
linchong.g_posY = 23
linchong.g_AIScript = 258
linchong.g_Title = "天雄星"
linchong.monster_groupId = 1
function linchong:OnDefaultEvent(selfId, targetId)
    self:BeginEvent(self.script_id)
    self:AddText("生死擂台不是这么好闯的，每战胜副本内每一个BOSS，都可以获得高级神秘物品，要注意危险哦!")
    self:AddNumText("#c00ff00决战 林充？", 6, 200)
    self:EndEvent()
    self:DispatchEventList(selfId, targetId)
end

function linchong:OnEventRequest(selfId, targetId, arg, index)
    ...
    if index == 200 then
        ...
        self:SetMonsterGroupID(MstId, self.monster_groupId)
...
function linchong:TipAllHuman(Str)
