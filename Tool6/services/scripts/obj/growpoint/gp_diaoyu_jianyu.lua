...
local gp_diaoyu_jianyu = class("gp_diaoyu_jianyu", script_base)
gp_diaoyu_jianyu.g_MainItemId = 20102014
gp_diaoyu_jianyu.g_AbilityId = 9
gp_diaoyu_jianyu.g_AbilityLevel = 2
function gp_diaoyu_jianyu:OnCreate(growPointType, x, y)
    local ItemCount = 0
    self:ItemBoxEnterScene(x, y, growPointType, define.QUALITY_MUST_BE_CHANGE, ItemCount)
end

function gp_diaoyu_jianyu:OnOpen(selfId, targetId)
...
function gp_diaoyu_jianyu:OnProcOver(selfId, targetId)
    local ret_1 = self:TryRecieveItem(selfId, self.g_MainItemId, define.QUALITY_MUST_BE_CHANGE)
    if ret_1 then
        self:Msg2Player(selfId, "你钓到一条箭鱼。", define.ENUM_CHAT_TYPE.MSG2PLAYER_PARA)
        local ABilityID = self:GetItemBoxRequireAbilityID(targetId)
