...
local gp_diaoyu_zhenzhuyu = class("gp_diaoyu_zhenzhuyu", script_base)
gp_diaoyu_zhenzhuyu.g_MainItemId = 20102023
gp_diaoyu_zhenzhuyu.g_AbilityId = 9
gp_diaoyu_zhenzhuyu.g_AbilityLevel = 11
function gp_diaoyu_zhenzhuyu:OnCreate(growPointType, x, y)
    local ItemCount = 0
    self:ItemBoxEnterScene(x, y, growPointType, define.QUALITY_MUST_BE_CHANGE, ItemCount)
end

function gp_diaoyu_zhenzhuyu:OnOpen(selfId, targetId)
...
function gp_diaoyu_zhenzhuyu:OnProcOver(selfId, targetId)
    local ret_1 = self:TryRecieveItem(selfId, self.g_MainItemId, define.QUALITY_MUST_BE_CHANGE)
    if ret_1 then
        self:Msg2Player(selfId, "你钓到一条珍珠鱼。", define.ENUM_CHAT_TYPE.MSG2PLAYER_PARA)
        local ABilityID = self:GetItemBoxRequireAbilityID(targetId)
