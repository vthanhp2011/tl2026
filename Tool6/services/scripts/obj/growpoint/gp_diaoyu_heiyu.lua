...
local gp_diaoyu_heiyu = class("gp_diaoyu_heiyu", script_base)
gp_diaoyu_heiyu.g_MainItemId = 20102006
gp_diaoyu_heiyu.g_AbilityId = 9
gp_diaoyu_heiyu.g_AbilityLevel = 6
function gp_diaoyu_heiyu:OnCreate(growPointType, x, y)
    local ItemCount = 0
    self:ItemBoxEnterScene(x, y, growPointType, define.QUALITY_MUST_BE_CHANGE, ItemCount)
end

function gp_diaoyu_heiyu:OnOpen(selfId, targetId)
...
function gp_diaoyu_heiyu:OnProcOver(selfId, targetId)
    local ret_1 = self:TryRecieveItem(selfId, self.g_MainItemId, define.QUALITY_MUST_BE_CHANGE)
    if ret_1 then
        self:Msg2Player(selfId, "你钓到一条黑鱼。", define.ENUM_CHAT_TYPE.MSG2PLAYER_PARA)
        local ABilityID = self:GetItemBoxRequireAbilityID(targetId)
