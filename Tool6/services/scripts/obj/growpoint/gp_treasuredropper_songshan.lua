...
local gp_treasuredropper_songshan = class("gp_treasuredropper_songshan", script_base)
gp_treasuredropper_songshan.g_DropNumTable = {
    {["num"] = 1, ["odd"] = 0.2},
    {["num"] = 2, ["odd"] = 0.4},
    {["num"] = 3, ["odd"] = 0.2},
    {["num"] = 4, ["odd"] = 0.15},
...
gp_treasuredropper_songshan.g_TickCreate_Msg = ""
gp_treasuredropper_songshan.g_DropTable = {
    {
        ["itemType"] = 1,
        ["odd"] = 0.293,
        ["numOdd"] = {
...
function gp_treasuredropper_songshan:GetTableIndexByOdd(tb)
...
function gp_treasuredropper_songshan:OnCreate(growPointType, x, y, dur)
    local ItemBoxId = nil
...
function gp_treasuredropper_songshan:OnTickCreateFinish(growPointType, tickCount)
    local sceneId = self:get_scene():get_id()
...
function gp_treasuredropper_songshan:OnOpen(selfId, targetId)
end

function gp_treasuredropper_songshan:OnRecycle(selfId, targetId)
    return 1
end

function gp_treasuredropper_songshan:OnProcOver(selfId, targetId)
end

function gp_treasuredropper_songshan:OpenCheck(selfId, AbilityId, AblityLevel)
    return define.OPERATE_RESULT.OR_OK
end

return gp_treasuredropper_songshan
