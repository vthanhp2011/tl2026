...
local gp_diaoyu_lianyu = class("gp_diaoyu_lianyu", script_base)
gp_diaoyu_lianyu.g_MainItemId = 20102003
gp_diaoyu_lianyu.g_AbilityId = 9
gp_diaoyu_lianyu.g_AbilityLevel = 3
function gp_diaoyu_lianyu:OnCreate(growPointType, x, y)
    local ItemCount = 0
    self:ItemBoxEnterScene(x, y, growPointType, define.QUALITY_MUST_BE_CHANGE, ItemCount)
end

function gp_diaoyu_lianyu:OnOpen(selfId, targetId)
...
function gp_diaoyu_lianyu:OnProcOver(selfId, targetId)
    local ret_1 = self:TryRecieveItem(selfId, self.g_MainItemId, define.QUALITY_MUST_BE_CHANGE)
    if ret_1 then
        self:Msg2Player(selfId, "你钓到一条鲢鱼。", define.ENUM_CHAT_TYPE.MSG2PLAYER_PARA)
        local ABilityID = self:GetItemBoxRequireAbilityID(targetId)
