...
local ai_hadaba_small = class("ai_hadaba_small", script_base)
--脚本号

--副本逻辑脚本号....
ai_hadaba_small.g_FuBenScriptId = 402276

--buff....
ai_hadaba_small.Buff_MianYi1	= 10472	--免疫一些负面效果....
ai_hadaba_small.Buff_MianYi2	= 10471	--免疫普通隐身....
ai_hadaba_small.Skill_A			=	1024
ai_hadaba_small.Buff_A			= 10230
ai_hadaba_small.Skill_B			=	1025
ai_hadaba_small.Buff_B			= 10231
ai_hadaba_small.Skill_C			=	1026
ai_hadaba_small.Buff_C			= 10232
ai_hadaba_small.Skill_D			=	1027
ai_hadaba_small.Buff_D			= 10233
ai_hadaba_small.Buff_E1			= 10234
ai_hadaba_small.Buff_E2			= 10235

--技能释放时间表....
ai_hadaba_small.UseSkillList =
{
	{ 20,  "A" },
	{ 25,  "B" },
	{ 40,  "A" },
...
--AI Index....
ai_hadaba_small.IDX_CombatTime		= 1	--进入战斗的计时器....用于记录已经进入战斗多长时间了....
ai_hadaba_small.IDX_UseSkillIndex	= 2	--接下来该使用技能表中的第几个技能....

ai_hadaba_small.IDX_CombatFlag 			= 1	--是否处于战斗状态的标志....
ai_hadaba_small.IDX_IsKuangBaoMode	= 2	--是否处于狂暴模式的标志....


--**********************************
--初始化....
--**********************************
function ai_hadaba_small:OnInit(selfId)
	print("ai_hadaba_small:OnInit")
	--重置AI....
...
--心跳....
--**********************************
function ai_hadaba_small:OnHeartBeat(selfId, nTick)
	--检测是不是死了....
...
--进入战斗....
--**********************************
function ai_hadaba_small:OnEnterCombat(selfId)
	print("ai_hadaba_small:OnEnterCombat")
	--简单版缥缈峰不需要加初始buff....
	--LuaFnSendSpecificImpactToUnit( selfId, selfId, selfId, ai_hadaba_small.Buff_MianYi1, 0 )
	--LuaFnSendSpecificImpactToUnit( selfId, selfId, selfId, ai_hadaba_small.Buff_MianYi2, 0 )
	--重置AI....
	self:ResetMyAI( selfId )
	--设置进入战斗状态....
...
--离开战斗....
--**********************************
function ai_hadaba_small:OnLeaveCombat(selfId)
	print("ai_hadaba_small:OnLeaveCombat")
	--重置AI....
	self:ResetMyAI( selfId )
...
--杀死敌人....
--**********************************
function ai_hadaba_small:OnKillCharacter(selfId, targetId)
...
--死亡....
--**********************************
function ai_hadaba_small:OnDie( selfId, killerId )
	print("ai_hadaba_small:OnDie selfId =", selfId, ";killerId =", killerId)
	--设置已经挑战过哈大霸....
...
--重置AI....
--**********************************
function ai_hadaba_small:ResetMyAI( selfId )
	--重置参数....
...
--BOSS使用技能....
--**********************************
function ai_hadaba_small:UseMySkill( selfId, skill )
	if skill == "A" then
...
--哪里走技能....对非少林玩家加buff....
--**********************************
function ai_hadaba_small:SkillA_NaLiZou( selfId )
...
--狂暴技能....
--**********************************
function ai_hadaba_small:SkillE_KuangBao( selfId )
	--取消BCD的buff....
	self:LuaFnCancelSpecificImpact( selfId, ai_hadaba_small.Buff_B )
	self:LuaFnCancelSpecificImpact( selfId, ai_hadaba_small.Buff_C )
	self:LuaFnCancelSpecificImpact( selfId, ai_hadaba_small.Buff_D )

	--加狂暴buff....
	self:LuaFnSendSpecificImpactToUnit( selfId, selfId, selfId, ai_hadaba_small.Buff_E1, 0 )
	self:LuaFnSendSpecificImpactToUnit( selfId, selfId, selfId, ai_hadaba_small.Buff_E2, 0 )
end

return ai_hadaba_small