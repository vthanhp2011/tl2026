...
eDynamicNPC_ThiefSoldier.g_LimitMembers = 3
...
function eDynamicNPC_ThiefSoldier:OnDefaultEvent(selfId, targetId)
    ...
    local CanAccept = self:OnAccept(selfId)
    if (1 == CanAccept) then self:LuaFnDeleteMonster(targetId) end
end

function eDynamicNPC_ThiefSoldier:OnEnumerate(caller, selfId, targetId, arg,
                                              index)
...
function eDynamicNPC_ThiefSoldier:MakeCopyScene(selfId, nearmembercount)
    local param0 = 4
    local param1 = 3
    local mems = {}
