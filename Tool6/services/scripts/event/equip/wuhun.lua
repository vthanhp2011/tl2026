...
function wuhun:AskWashKfsSkill(selfId, nKfsPos)
    ...
    if self:LuaFnMtl_GetCostNum(selfId, KfsItem_YiHunShi) < 1 then
    ...
    self:LuaFnMtl_CostMaterial(selfId, 1, KfsItem_YiHunShi)
end
...
function wuhun:KfsSkillUp(selfId, nKfsPos, _, nIndex)
    ...
    if self:LuaFnMtl_GetCostNum(selfId, {needItem}) < 1 then
    ...
    --数据存储完毕，恭喜玩家了
    if not self:LuaFnMtl_CostMaterial(selfId, 1, {needItem}) then
...
function wuhun:KfsReWashGrowEx(selfId, nKfsPos)
    ...
    if self:LuaFnMtl_GetCostNum(selfId, KfsItem_HuiTianShenShi) < 1 then
        return
    end
    local nMoneySelf = self:GetMoneyJZ(selfId) + self:GetMoney(selfId)
    if nMoneySelf < 50000 then
    ...
    self:LuaFnMtl_CostMaterial(selfId, 1, KfsItem_HuiTianShenShi)
end
...
function wuhun:RunHunShiCompound_New(selfId, targetId, ...)
    ...
    if operate == 2 then
        ...
        self:LuaFnCostMoneyWithPriority(selfId, needMoney)
        self:LuaFnDelAvailableItem(selfId, item, num)
...
local hunbingzhu = {
    [1] = {
        item = 20310117, count = 5, target = 20310118, cost_money = 5000
    },
    [2] = {
        item = 20310118, count = 5, target = 20310119, cost_money = 5000
    },
    [3] = {
        item = 20310119, count = 5, target = 20310120, cost_money = 10000
    },
    [4] = {
        item = 20310120, count = 5, target = 20310121, cost_money = 10000
    },
}
