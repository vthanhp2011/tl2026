...
function judge_aptitude:FinishAdjust(selfId, ItemPos)
    ...
    local base = Item:get_base_config()
...
function judge_aptitude:FinishReAdjust(selfId, ItemPos)
    ...
    local itemCount = self:LuaFnGetAvailableItemCount(selfId, 30900061)
	local itemCount2 = self:LuaFnGetAvailableItemCount(selfId, 30505267)
	if itemCount < 1 and itemCount2 < 1 then
        self:notify_tips(selfId, "您缺少天晶砂或水晶锉。")
		return
	end
    local equip_level = base.level
    local money = self:GetMoney(selfId) + self:GetMoneyJZ(selfId)
	...
	--找绑定天晶砂
	local ShaPos = self:GetBagPosByItemSnAvailableBind(selfId, 30900061, true)
	...
	if ShaPos == -1 then --找绑定水晶锉
		ShaPos2 = self:GetBagPosByItemSnAvailableBind(selfId, 30505267, true)
	end
	if ret then
		if ShaPos2 ~= -1 then
			needBind = true
	...
	if ShaPos == -1 and ShaPos2 == -1 then --找天晶砂
		ShaPos = self:GetBagPosByItemSnAvailableBind(selfId, 30900061, false)
	end
	if ShaPos == -1 and ShaPos2 == -1 then --找水晶锉
		ShaPos2 = self:GetBagPosByItemSnAvailableBind(selfId, 30505267, false)
	end
	local ShaInfo = 0
	-- 扣天晶砂或水晶锉....
	if ShaPos ~= -1 then
