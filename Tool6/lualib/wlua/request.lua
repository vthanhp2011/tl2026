...
local max_request_body_size = 1024 * 1024
...
function M:new(id, interface)
    ...
    skynet.logi("code =", code, ";method =", method, ";headers =", table.tostr(headers), ";body_raw =", body_raw)
    if not code then
        if url == sockethelper.socket_error then
            skynet.logw("Socket closed. id:", id)
        else
            skynet.logw("Request error. id:", id, ",url:", url)
        end
        return
    end

    skynet.logd("req. url:", url, ", method:", method, ", headers:", headers)
