...
static void
push_data(lua_State *L, int fd, struct header header, void *buffer, int clone) {
	if (clone) {
		int size = header.sz;
	...
	if (++q->tail >= q->cap)
		q->tail -= q->cap;
	np->id = fd;
	memcpy(&(np->header), &header, sizeof(struct header));
	np->buffer = buffer;
	if (q->head == q->tail) {
		expand_queue(L, q);
	}
...
read_header(uint8_t * buffer) {
	struct header header;
	memcpy(&header, buffer, 6);
	return header;
}

static void
push_more(lua_State *L, int fd, uint8_t *buffer, int size) {
	int headersz = sizeof(struct header);
	if (size < headersz ) {
		struct uncomplete * uc = save_uncomplete(L, fd);
		uc->read = size - headersz;
	...
	int pack_size = header.sz;
	if (size < pack_size) {	
		struct uncomplete * uc = save_uncomplete(L, fd);
		uc->read = size;
		uc->pack.header.sz = header.sz;
		memcpy(&(uc->header), &header, headersz);
		uc->pack.buffer = skynet_malloc(pack_size);
		memcpy(uc->pack.buffer, buffer, size);
		return;
	}
	push_data(L, fd, header, buffer, 1);
...
filter_data_(lua_State *L, int fd, uint8_t * buffer, int size) {
	size_t headersz = sizeof(struct header);
	struct queue *q = lua_touserdata(L,1);
	struct uncomplete * uc = find_uncomplete(q, fd);
	if (uc) {
		// fill uncomplete
		if (uc->read < 0) {
			// read size
			int header_read = headersz + uc->read;
			int header_need = -1 * uc->read;
			int read_size = header_need > size ? size : header_need;
			memcpy((uint8_t *)&(uc->header) + header_read, buffer, read_size);
			uc->read = uc->read + read_size;
			size = size - read_size;
			buffer += read_size;
		}
		if (uc->read < 0){
		...
		if (uc->read == 0){
			uc->pack.buffer = skynet_malloc(uc->header.sz);
		}
		int need = uc->header.sz - uc->read;
		if (size < need) {
		...
		if (size == 0) {
			...
			lua_pushinteger(L, uc->header.id);
			lua_pushinteger(L, uc->header.sz);
			lua_pushinteger(L, uc->header.idx);
			lua_pushlightuserdata(L, uc->pack.buffer);
			skynet_free(uc);
			return 7;
		}
		// more data
		push_data(L, fd, uc->header, uc->pack.buffer, 0);
		skynet_free(uc);
		push_more(L, fd, buffer, size);
		...
		if (size < pack_size) {
			struct uncomplete * uc = save_uncomplete(L, fd);
			uc->read = size;
			memcpy(&(uc->pack.header), &header, headersz);
			memcpy(&(uc->header), &header, headersz);
			uc->pack.buffer = skynet_malloc(pack_size);
			memcpy(uc->pack.buffer, buffer, size);
			return 1;
		...
		if (size == pack_size) {
			...
			lua_pushvalue(L, lua_upvalueindex(TYPE_DATA));
			lua_pushinteger(L, fd);
			lua_pushinteger(L, header.id);
			lua_pushinteger(L, header.sz);
			lua_pushinteger(L, header.idx);
...
lpop(lua_State *L) {
	...
	lua_pushinteger(L, np->header.id);
	lua_pushinteger(L, np->header.sz);
	lua_pushinteger(L, np->header.idx);
...
static inline void
write_header(uint8_t * buffer, struct header header){
...
lpack(lua_State *L) {
	...
	uint16_t headersz = sizeof(struct header);
	header.id = (uint16_t)lua_tointeger(L, 2);
	header.sz = len;
	header.idx= (uint16_t)lua_tointeger(L, 3);
	uint8_t * buffer = skynet_malloc(len + headersz);
	write_header(buffer, header);
	memcpy(buffer + headersz, ptr, len);

	lua_pushlightuserdata(L, buffer);
	lua_pushinteger(L, len + headersz);
