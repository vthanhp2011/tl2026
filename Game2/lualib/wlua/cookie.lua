-- Fork from https://github.com/cloudflare/lua-resty-cookie
...
local _ok, clear_tab = pcall(require, "table.clear")
if not _ok then
...
local function get_cookie_table(text_cookie)
    if type(text_cookie) ~= "string" then
        skynet.loge(format("expect text_cookie to be \"string\" but found %s",
                type(text_cookie)))
        return {}
    end
...
function _M.new(self, c)
    local _cookie = c.req.headers["cookie"]
    return setmetatable({
        _cookie = _cookie,
        set_cookie_table = new_tab(4, 0),
        _context = c,
    },
    { __index = self })
end
...
local function bake(cookie)
    ...
    if (cookie.samesite) then
        ...
        if (samesite ~= "Strict" and samesite ~= "Lax" and samesite ~= "None") then
            skynet.logw("SameSite value must be 'Strict', 'Lax' or 'None'")
            cookie.samesite = nil
        end
    end
...
function _M.set(self, cookie)
    ...
    local set_cookie = self._context.res:get_header('Set-Cookie')
    ...
    elseif set_cookie_type == "table" then
        ...
        self._context.res:set_header('Set-Cookie', cookie_str)
    end
    return true
end
