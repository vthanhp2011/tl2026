...
local handle_web = require "wlua.handle"
...
local enter_cache = cache_help:new(10,function(fd,addr) 
	if enter_map[fd] then
		skynet.loge("enter_timer_out:",fd,addr)
...
local function handle_func(req)
	...
	if req_cnt > second_req_limit then
		skynet.logw("request so buzy ",fd,req_cnt,second_req_limit)
		return 400
	end
...
function SOCKET.enter(fd, ip,port,master_id)
	if enter_cache:get_cache(fd) then
		skynet.logw("repeat enter fd:",fd,ip,port)
	end
	...
	if not is_ok then
		skynet.logw("handle_web err ",handle)
		clear_fd(fd)
	...
	skynet.fork(function()
		while keep_alive do
			if socket.disconnected(fd) or socket.invalid(fd) then
				skynet.logw("disconnect:fd",fd,socket.disconnected(fd),socket.invalid(fd))
				break
			end
			...
			if not is_ok then
				skynet.logw("read_request err ",ret)
				break
			end
			...
			if not is_ok then
				skynet.loge("handle_response err ",ret)
				break
			end
...
function CMD.fix_exit()
	skynet.loge("web_agent_module exit begin!")
	for fd,info in pairs(fd_info_map) do
		skynet.loge("web_agent_module exit id:",fd)
		skynet.fork(close_fd,fd)
	end
	skynet.loge("web_agent_module exit end!")
end
