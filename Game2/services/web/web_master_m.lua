...
local function check_addr(ip,port,fd)
	if fd_agent_map[fd] then
		skynet.logw("rpeat fd:",fd)
		return false
	end

	if client_num >= max_client then
		skynet.logw("client_num full",max_client)
		return false
	end

	if limit_conn_sum_map[ip] and limit_conn_sum_map[ip] >= second_conn_limit then
		skynet.logw("connect so frequently ",ip,limit_conn_sum_map[ip],second_conn_limit)
		return false
	end
	...
	if lock_kick_num >= keep_live_limit then
		skynet.logw("beyond keep live limit ",keep_live_num,lock_kick_num,keep_live_limit,client_num)
		return false
	end

	if black_ip_map[ip] then
		skynet.logw("black_ip ",ip)
		return false
	end
	return true
end
...
local function connect_succ(ip,port,fd,agent_id)
	...
	if #keep_live_list >= keep_live_limit then
		...
		if rm_fd then
			...
			skynet.logw("connect_succ rm_fd_agent_id err",rm_fd)
			else
			...
			skynet.logw("connect_succ beyond keep_live_limit err")
		end
	end
...
local agent_clients = {}
local agent_cur_index = 1

local function balance_call(...)
	agent_cur_index = agent_cur_index + 1
	agent_cur_index = agent_cur_index > #agent_clients and 1 or agent_cur_index
	local agent = agent_clients[agent_cur_index]
	return skynet.call(agent, "lua", ...)
end
...
function CMD.init(args)
	...
	skynet.loge(string.format("Listening %s://%s:%s max_client[%s] id[%s]", protocol, host, port, max_client,listen_fd))
	for i = 1, 10 do
		agent_client = skynet.newservice("web_agent")
		skynet.call(agent_client, "lua", "init", { protocol = protocol, dispatch = "webapp_dispatch"})
		table.insert(agent_clients, agent_client)
	end
	socket.start(listen_fd, function(fd, addr)
		...
		if not ip or not port then
			socket.close_fd(fd)
			skynet.logw("connect err ",addr,ip,port)
			return
		end

		if not check_addr(ip,port,fd) then
			socket.close_fd(fd)
			skynet.logw("unsafe addr ",addr)
			return
		end
		local agent_id = balance_call('socket','enter',fd,ip,port,SELF_ADDRESS)
