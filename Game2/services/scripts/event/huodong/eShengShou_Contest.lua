...
--心跳函数
--**********************************
function eShengShou_Contest:OnTimer(actId, uTime )
	--检测活动是否过期
...
--事件交互入口
--**********************************
function eShengShou_Contest:OnDefaultEvent(actId, param1, param2, param3, param4, param5 )
...
--检测是否可以打开大宝箱....
--**********************************
function eShengShou_Contest:CheckOpenBigBox(selfId )
	--宝箱是否存在....
	if self.g_IsBigBoxExist == 0 then
		return 0, "(ERROR)"
	end
	--如果有人正在开大宝箱....
	if self.g_IsBigBoxOpening == 1 then
		--如果是自己在开....
		if self.g_OpeningPlayerName == self:LuaFnGetName(selfId ) then
			return 1, "(ERROR)"
		end

		--如果是别人在开并且他已经超时了....则让位给我来开....
		local NowTime = self:LuaFnGetCurrentTime()
		if (NowTime - self.g_PlayerOpeningTime) > 150 then
			self.g_PlayerOpeningTime = NowTime
			self.g_OpeningPlayerName = self:LuaFnGetName(selfId)
			return 1, "(ERROR)"
		else
			return -1, self.g_OpeningPlayerName
		end
	end

	--没有人在开大宝箱....
	self.g_IsBigBoxOpening = 1
	self.g_PlayerOpeningTime = self:LuaFnGetCurrentTime()
	self.g_OpeningPlayerName = self:LuaFnGetName(selfId)
	return 1, "(ERROR)"
end
...
--**********************************
function eShengShou_Contest:OnCancelOpen()
	self.g_IsBigBoxOpening = 0
	self.g_OpeningPlayerName = "(ERROR)"
	self.g_PlayerOpeningTime = 0
end
...
function eShengShou_Contest:OnBigBoxOpen( selfId, activatorId )
	...
	if rand <= 30 then
		--开出技能书....
		self:GiveDropBox(selfId, activatorId )
	else
		--开出一堆掉落包....
		self:GiveSkillBook(selfId, activatorId )
	end
...
--大宝箱被打开事件_给技能书....
--**********************************
function eShengShou_Contest:GiveSkillBook(selfId, activatorId )
	--随机给玩家一本技能书....并公告....
	local rand = math.random(100)
	local numBooks
	local randBook
...
--大宝箱被打开事件_开出BOSS....
--**********************************
function eShengShou_Contest:GiveBOSS(selfId, activatorId )
	--刷BOSS....
	local MstId
...
--大宝箱被打开事件_掉落一堆掉落包....
--**********************************
function eShengShou_Contest:GiveDropBox(selfId, activatorId )
	--计算总权重....
	local totalOdds = 0
...
function eShengShou_Contest:OnPlayerPickUpItemInBoar(selfId, itemId, bagidx )
	...
	if itemId == 40004429 then
		...
		local str = string.format(fmt, gbk.fromutf8(playerName), gbk.fromutf8(transfer))
