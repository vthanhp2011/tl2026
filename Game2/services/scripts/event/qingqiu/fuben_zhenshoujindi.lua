...
function fuben_zhenshoujindi:CheckAccept(selfId, targetId, nType)
    ...
    if self:LuaFnHasTeam(selfId) then
        if not self:LuaFnIsTeamLeader(selfId) == 0 then
...
function fuben_zhenshoujindi:MakeCopyScene(selfId, nType)
...
function fuben_zhenshoujindi:OnPlayerEnter(selfId)
    ...
    self:AddTodayEnterTime(selfId, nFubenType)
    if nFubenType == 0 then
        self:DungeonDone(selfId, 12)
    else
        self:DungeonDone(selfId, 13)
    end
