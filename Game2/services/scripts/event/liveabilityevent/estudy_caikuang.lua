...
local estudy_caikuang = class("estudy_caikuang", script_base)
estudy_caikuang.script_id = 713508
estudy_caikuang.g_MessageStudy = "如果你达到%d级并且肯花费#{_EXCHG%d}就可以学会采矿技能。你决定学习么？"
estudy_caikuang.g_AbilityID = define.ABILITY_CAIKUANG
estudy_caikuang.g_AbilityName = "采矿"
function estudy_caikuang:OnDefaultEvent(selfId, targetId, ButtomNum, g_Npc_ScriptId, bid)
    ...
    if ButtomNum == 0 then
        local ret, demandMoney, demandExp, limitAbilityExp, limitAbilityExpShow, currentLevelAbilityExpTop, limitLevel, extraMoney, extraExp = self:LuaFnGetAbilityLevelUpConfig2(define.ABILITY_CAIKUANG, 1)
        if ret then
            self:BeginEvent(self.script_id)
...
function estudy_caikuang:OnEnumerate(caller, selfId, targetId, bid)
    if bid then
    ...
    local ret = self:LuaFnGetAbilityLevelUpConfig(define.ABILITY_CAIKUANG, 1)
    if ret then
        caller:AddNumTextWithTarget(self.script_id, "学习" .. self.g_AbilityName .. "技能", 12, 0)
    end
    return
...
function estudy_caikuang:CheckAccept()

end

function estudy_caikuang:OnAccept()

end

function estudy_caikuang:StudyInCity(selfId, targetId, ButtomNum, g_Npc_ScriptId, bid)
    if bid then
        if 0 == ButtomNum then
