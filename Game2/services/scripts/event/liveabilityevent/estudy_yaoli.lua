...
local estudy_yaoli = class("estudy_yaoli", script_base)
estudy_yaoli.script_id = 713527
estudy_yaoli.g_MessageStudy = "如果你达到%d级并且肯花费#{_EXCHG%d}就可以学会药理技能。你决定学习么？"
estudy_yaoli.g_AbilityID = define.ABILITY_YAOLI
estudy_yaoli.g_AbilityName = "药理"
function estudy_yaoli:OnDefaultEvent(selfId, targetId, ButtomNum, g_Npc_ScriptId, bid)
    ...
    if ButtomNum == 0 then
        local ret, demandMoney, demandExp, limitAbilityExp, limitAbilityExpShow, currentLevelAbilityExpTop, limitLevel, extraMoney, extraExp = self:LuaFnGetAbilityLevelUpConfig2(define.ABILITY_YAOLI, 1)
        if ret then
            self:BeginEvent(self.script_id)
...
function estudy_yaoli:OnEnumerate(caller, selfId, targetId, bid)
    if bid then
    ...
    local ret = self:LuaFnGetAbilityLevelUpConfig(define.ABILITY_YAOLI, 1)
    if ret then
        caller:AddNumTextWithTarget(self.script_id, "学习" .. self.g_AbilityName .. "技能", 12, 0)
    end
    return
...
function estudy_yaoli:CheckAccept()

end

function estudy_yaoli:OnAccept()

end

function estudy_yaoli:StudyInCity(selfId, targetId, ButtomNum, g_Npc_ScriptId, bid)
    if bid then
        if 0 == ButtomNum then
