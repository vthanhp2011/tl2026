...
local estudy_zhiyao = class("estudy_zhiyao", script_base)
estudy_zhiyao.script_id = 713503
estudy_zhiyao.g_MessageStudy = "如果你达到%d级并且肯花费#{_EXCHG%d}就可以学会制药技能。你决定学习么？"
estudy_zhiyao.g_AbilityID = define.ABILITY_ZHIYAO
estudy_zhiyao.g_AbilityName = "制药"
function estudy_zhiyao:OnDefaultEvent(selfId, targetId, ButtomNum, g_Npc_ScriptId, bid)
    ...
    if ButtomNum == 0 then
        local ret, demandMoney, demandExp, limitAbilityExp, limitAbilityExpShow, currentLevelAbilityExpTop, limitLevel, extraMoney, extraExp = self:LuaFnGetAbilityLevelUpConfig2(define.ABILITY_ZHIYAO, 1)
        if ret then
            self:BeginEvent(self.script_id)
...
function estudy_zhiyao:OnEnumerate(caller, selfId, targetId, bid)
    if bid then
    ...
    local ret = self:LuaFnGetAbilityLevelUpConfig(define.ABILITY_ZHIYAO, 1)
    if ret then
        caller:AddNumTextWithTarget(self.script_id, "学习" .. self.g_AbilityName .. "技能", 12, 0)
    end
    return
...
function estudy_zhiyao:CheckAccept()

end

function estudy_zhiyao:OnAccept()

end

function estudy_zhiyao:StudyInCity(selfId, targetId, ButtomNum, g_Npc_ScriptId, bid)
    if bid then
        if 0 == ButtomNum then
