...
function GemMelting:OnGemMelting(selfId, GemItemIndex, MatItemPos)
    ...
    local param = {}
    local _, bag_index = human_item_logic:create_multi_item_to_bag(param, obj, product_id, 1, item:is_bind(), 0)
    if bag_index ~= define.INVAILD_ID then
        for _, bi in pairs(bag_indexs) do
            local logparam = {}
            human_item_logic:dec_item_lay_count(logparam, obj, bi.bag_index, bi.count)
        end
        local logparam = {}
        human_item_logic:dec_item_lay_count(logparam, obj, MatItemPos, 1)
