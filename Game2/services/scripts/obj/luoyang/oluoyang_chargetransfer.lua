...
local oluoyang_chargetransfer = class("oluoyang_chargetransfer", script_base)
oluoyang_chargetransfer.script_id = 000156
oluoyang_chargetransfer.g_name = "车传义"
oluoyang_chargetransfer.g_transfer_target = {
    [1] = {["x"] = 235, ["z"] = 156, ["scene_num"] = 1},
    [2] = {["x"] = 246, ["z"] = 106, ["scene_num"] = 2},
    [3] = {["x"] = 294, ["z"] = 90, ["scene_num"] = 186},
    [4] = {["x"] = 206, ["z"] = 266, ["scene_num"] = 34},
    [5] = {["x"] = 186, ["z"] = 43, ["scene_num"] = 28},
    [6] = {["x"] = 158, ["z"] = 113, ["scene_num"] = 22}
}

oluoyang_chargetransfer.g_transfer_cost = 5000
function oluoyang_chargetransfer:OnDefaultEvent(selfId, targetId)
    self:UpdateEventList(selfId, targetId)
end

function oluoyang_chargetransfer:UpdateEventList(selfId, targetId)
    self:BeginEvent(self.script_id)
    self:AddText("#{SFCS_80828_01}")
    self:AddNumText("苏州", 9, 1)
    self:AddNumText("大理", 9, 2)
    self:AddNumText("楼兰", 9, 3)
    self:AddNumText("南海", 9, 4)
    self:AddNumText("南诏", 9, 5)
    self:AddNumText("长白山", 9, 6)
    self:EndEvent()
    self:DispatchEventList(selfId, targetId)
end

function oluoyang_chargetransfer:OnEventRequest(selfId, targetId, arg, index)
    ...
    if self:GetTeamId(selfId) >= 0 and self:IsTeamFollow(selfId) and
        ...
        self:BeginEvent(self.script_id)
        self:AddText(
            "  非常抱歉，由于路途太过遥远，我们这里运输能力有限，所以不接受组队传送，请您离开队伍单独前来吧！")
        self:EndEvent()
        self:DispatchEventList(selfId, targetId)
        return
    end
    ...
    elseif id == 100 then
        ...
        local minLevel = 10
        if index == 3 then minLevel = 75 end
        if self:LuaFnGetDRideFlag(selfId) then
            local objId = self:LuaFnGetDRideTargetID(selfId)
            if objId ~= -1 and self:GetLevel(objId) < minLevel then
                local Tip = string.format("#{CQTS_90227_1}%d#{CQTS_90227_2}",
                                        minLevel)
        ...
        if (nMoneyJZ + nMoney) >= self.g_transfer_cost then
            if self:LuaFnCostMoneyWithPriority(selfId, self.g_transfer_cost) ==
                -1 then
                self:BeginEvent(self.script_id)
                self:AddText("收费失败！")
                self:EndEvent()
                ...
                self:CallScriptFunction((400900), "TransferFunc", selfId,
                                        scene_num, pos_x, pos_z, minLevel)
            end
        else
            self:BeginEvent(self.script_id)
            self:AddText("金钱不足")
...
function oluoyang_chargetransfer:NotifyFailTips(selfId, targetId, Tip)
    self:BeginEvent(self.script_id)
    self:AddText(Tip)
    self:EndEvent()
    self:DispatchEventList(selfId, targetId)
...
return oluoyang_chargetransfer
