...
local olujunyi = class("olujunyi", script_base)
olujunyi.script_id = 760704
olujunyi.g_FuBenScriptId = 892009
function olujunyi:OnDefaultEvent(selfId, targetId)
    self:BeginEvent(self.script_id)
    self:AddText("胆敢挑战我们十二煞星，真是活的不耐烦了！")
    self:AddNumText("决战卢君逸", 6, 1)
    self:EndEvent()
    self:DispatchEventList(selfId, targetId)
end

function olujunyi:OnEventRequest(selfId, targetId, arg, index)
...
function olujunyi:OnSJZTimer(step, data1, data2)
    if 7 == step then
    ...
    if 2 == step then
        self:CallScriptFunction(self.g_FuBenScriptId, "TipAllHuman", "卢君逸已经加入战斗，请大侠准备迎战！")
        self:CallScriptFunction(self.g_FuBenScriptId, "DeleteBOSS", "lujunyi_NPC")
        return
    end
    if 1 == step then
        self:CallScriptFunction(self.g_FuBenScriptId, "CreateBOSS", "lujunyi_BOSS", -1, -1)
        return
    end
end
