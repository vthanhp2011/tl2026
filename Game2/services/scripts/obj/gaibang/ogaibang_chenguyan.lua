...
local ogaibang_chenguyan = class("ogaibang_chenguyan", script_base)
ogaibang_chenguyan.script_id = 10000
local MenPaiPet =
{
...
function ogaibang_chenguyan:OnDefaultEvent(selfId, targetId)
	local menpai = self:GetMenPai(selfId)
    self:BeginEvent(self.script_id)
        self:AddText("  我是陈孤雁，阁下来找我有何事指教？")
        if menpai == 9 then
...
function ogaibang_chenguyan:OnEventRequest(selfId, targetId, arg, index)
	...
	if index == 0 then
		...
		if nMenPai == 2 then
			self:MsgBox(selfId,targetId,"#{event_gaibang_0001}")
			return
		end
		self:BeginEvent(self.script_id)
		self:AddText(" #{MenpaiInfo_002}")
		self:AddNumText("我确定要拜入丐帮", 6,3)
		self:AddNumText("我暂时还不想拜入门派", 8,1000)
		self:EndEvent()
		self:DispatchEventList(selfId, targetId)
	elseif index == 1 then
		self:MsgBox(selfId, targetId, " #{OBJ_gaibang_0002}")
	elseif index == 6 then
		self:MsgBox(selfId, targetId, " #{MPJN_090728_02}")
	elseif index == 3 then
		...
		if self:GetMenPai(selfId) == 9 then
			self:LuaFnJoinMenpai(selfId,2)
			self:LuaFnSetXinFaLevel(selfId,13,10)
			self:LuaFnSetXinFaLevel(selfId,14,1)
			self:LuaFnSetXinFaLevel(selfId,15,1)
			self:LuaFnSetXinFaLevel(selfId,16,1)
			self:LuaFnSetXinFaLevel(selfId,17,10)
			self:LuaFnSetXinFaLevel(selfId,18,1)
			self:AddItemListToHuman(selfId)
			self:notify_tips(selfId, "你已经加入丐帮！")
			self:notify_tips(selfId, "你获得了" .. self:GetItemName(10124002) .. "。")
	...
	elseif index == 100 then
		...
		self:BeginAddItem()
		self:AddItem(10431210, 1,true)
		self:AddItem(10443210, 1,true)
		local ret = self:EndAddItem(selfId)
		if not ret then
			self:MsgBox(selfId, targetId, "  你的背包空间不够了，整理后再来找我。")
			return
		...
		self:AddItemListToHuman(selfId)
		self:notify_tips(selfId, "你获得了" .. self:GetItemName(10431210) .. "。")
		self:notify_tips(selfId, "你获得了" .. self:GetItemName(10443210) .. "。")
	...
	elseif index == 102 then
		...
		if self:GetItemCount(selfId, 10124002) > 0 then
...
function ogaibang_chenguyan:MsgBox(selfId, targetId, msg)
	self:BeginEvent(self.script_id)
	self:AddText(msg)
	self:EndEvent()
	self:DispatchEventList(selfId, targetId)
end
return ogaibang_chenguyan