...
local gp_diaoyu_yinyu = class("gp_diaoyu_yinyu", script_base)
gp_diaoyu_yinyu.g_MainItemId = 20102009
gp_diaoyu_yinyu.g_AbilityId = 9
gp_diaoyu_yinyu.g_AbilityLevel = 9
function gp_diaoyu_yinyu:OnCreate(growPointType, x, y)
    local ItemCount = 0
    self:ItemBoxEnterScene(x, y, growPointType, define.QUALITY_MUST_BE_CHANGE, ItemCount)
end

function gp_diaoyu_yinyu:OnOpen(selfId, targetId)
...
function gp_diaoyu_yinyu:OnProcOver(selfId, targetId)
    local ret_1 = self:TryRecieveItem(selfId, self.g_MainItemId, define.QUALITY_MUST_BE_CHANGE)
    if ret_1 then
        self:Msg2Player(selfId, "你钓到一条银鱼。", define.ENUM_CHAT_TYPE.MSG2PLAYER_PARA)
        local ABilityID = self:GetItemBoxRequireAbilityID(targetId)
