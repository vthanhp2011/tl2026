...
local gp_diaoyu_suoyu = class("gp_diaoyu_suoyu", script_base)
gp_diaoyu_suoyu.g_MainItemId = 20102019
gp_diaoyu_suoyu.g_AbilityId = 9
gp_diaoyu_suoyu.g_AbilityLevel = 7
function gp_diaoyu_suoyu:OnCreate(growPointType, x, y)
    local ItemCount = 0
    self:ItemBoxEnterScene(x, y, growPointType, define.QUALITY_MUST_BE_CHANGE, ItemCount)
end

function gp_diaoyu_suoyu:OnOpen(selfId, targetId)
...
function gp_diaoyu_suoyu:OnProcOver(selfId, targetId)
    local ret_1 = self:TryRecieveItem(selfId, self.g_MainItemId, define.QUALITY_MUST_BE_CHANGE)
    if ret_1 then
        self:Msg2Player(selfId, "你钓到一条梭鱼。", define.ENUM_CHAT_TYPE.MSG2PLAYER_PARA)
        local ABilityID = self:GetItemBoxRequireAbilityID(targetId)
