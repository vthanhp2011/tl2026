...
local gp_diaoyu_douyu = class("gp_diaoyu_douyu", script_base)
gp_diaoyu_douyu.g_MainItemId = 20102007
gp_diaoyu_douyu.g_AbilityId = 9
gp_diaoyu_douyu.g_AbilityLevel = 7
function gp_diaoyu_douyu:OnCreate(growPointType, x, y)
    local ItemCount = 0
    self:ItemBoxEnterScene(x, y, growPointType, define.QUALITY_MUST_BE_CHANGE, ItemCount)
end

function gp_diaoyu_douyu:OnOpen(selfId, targetId)
...
function gp_diaoyu_douyu:OnProcOver(selfId, targetId)
    local ret_1 = self:TryRecieveItem(selfId, self.g_MainItemId, define.QUALITY_MUST_BE_CHANGE)
    if ret_1 then
        self:Msg2Player(selfId, "你钓到一条斗鱼。", define.ENUM_CHAT_TYPE.MSG2PLAYER_PARA)
        local ABilityID = self:GetItemBoxRequireAbilityID(targetId)
