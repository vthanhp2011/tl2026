...
local gp_diaoyu_biantouyu = class("gp_diaoyu_biantouyu", script_base)
gp_diaoyu_biantouyu.g_MainItemId = 20102016
gp_diaoyu_biantouyu.g_AbilityId = 9
gp_diaoyu_biantouyu.g_AbilityLevel = 4
function gp_diaoyu_biantouyu:OnCreate(growPointType, x, y)
    local ItemCount = 0
    self:ItemBoxEnterScene(x, y, growPointType, define.QUALITY_MUST_BE_CHANGE, ItemCount)
end

function gp_diaoyu_biantouyu:OnOpen(selfId, targetId)
...
function gp_diaoyu_biantouyu:OnProcOver(selfId, targetId)
    local ret_1 = self:TryRecieveItem(selfId, self.g_MainItemId, define.QUALITY_MUST_BE_CHANGE)
    if ret_1 then
        self:Msg2Player(selfId, "你钓到一条扁头鱼。", define.ENUM_CHAT_TYPE.MSG2PLAYER_PARA)
        local ABilityID = self:GetItemBoxRequireAbilityID(targetId)
