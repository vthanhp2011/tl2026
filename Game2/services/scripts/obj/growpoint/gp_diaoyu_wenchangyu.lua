...
local gp_diaoyu_wenchangyu = class("gp_diaoyu_wenchangyu", script_base)
gp_diaoyu_wenchangyu.g_MainItemId = 20102024
gp_diaoyu_wenchangyu.g_AbilityId = 9
gp_diaoyu_wenchangyu.g_AbilityLevel = 12
function gp_diaoyu_wenchangyu:OnCreate(growPointType, x, y)
    local ItemCount = 0
    self:ItemBoxEnterScene(x, y, growPointType, define.QUALITY_MUST_BE_CHANGE, ItemCount)
end

function gp_diaoyu_wenchangyu:OnOpen(selfId, targetId)
...
function gp_diaoyu_wenchangyu:OnProcOver(selfId, targetId)
    local ret_1 = self:TryRecieveItem(selfId, self.g_MainItemId, define.QUALITY_MUST_BE_CHANGE)
    if ret_1 then
        self:Msg2Player(selfId, "你钓到一条文昌鱼。", define.ENUM_CHAT_TYPE.MSG2PLAYER_PARA)
        local ABilityID = self:GetItemBoxRequireAbilityID(targetId)
