...
local gp_caiyao_duzhong = class("gp_caiyao_duzhong", script_base)
gp_caiyao_duzhong.g_MainItemId = 20101011
gp_caiyao_duzhong.g_SubItemId = 20304008
gp_caiyao_duzhong.g_AbilityId = 8
gp_caiyao_duzhong.g_AbilityLevel = 4
function gp_caiyao_duzhong:OnCreate(growPointType, x, y)
...
function gp_caiyao_duzhong:OnOpen(selfId, targetId)
...
function gp_caiyao_duzhong:OnRecycle(selfId, targetId)
    ABilityID = self:GetItemBoxRequireAbilityID(targetId)
    self:CallScriptFunction(define.ABILITYLOGIC_ID, "GainExperience", selfId, ABilityID, self.g_AbilityLevel)
    return 1
end

function gp_caiyao_duzhong:OnProcOver(selfId, targetId)
    local ABilityID = self:GetItemBoxRequireAbilityID(targetId)
    self:CallScriptFunction(define.ABILITYLOGIC_ID, "EnergyCostCaiJi", selfId, ABilityID, self.g_AbilityLevel)
    return 0
end

function gp_caiyao_duzhong:OpenCheck(selfId, AbilityId, AbilityLevel)
