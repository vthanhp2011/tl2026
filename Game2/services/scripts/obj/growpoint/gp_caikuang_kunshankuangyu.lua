...
local gp_caikuang_kunshankuangyu = class("gp_caikuang_kunshankuangyu", script_base)
gp_caikuang_kunshankuangyu.g_MainItemId = 20103118
gp_caikuang_kunshankuangyu.g_SubItemId = 50112004
gp_caikuang_kunshankuangyu.g_Byproduct = {20103122, 20103126, 20103130, 20103134}

gp_caikuang_kunshankuangyu.g_AbilityId = 7
gp_caikuang_kunshankuangyu.g_AbilityLevel = 9
function gp_caikuang_kunshankuangyu:OnCreate(growPointType, x, y)
...
function gp_caikuang_kunshankuangyu:OnOpen(selfId, targetId)
...
function gp_caikuang_kunshankuangyu:OnRecycle(selfId, targetId)
    local ABilityID = self:GetItemBoxRequireAbilityID(targetId)
    self:CallScriptFunction(define.ABILITYLOGIC_ID, "GainExperience", selfId, ABilityID, self.g_AbilityLevel)
    return 1
end

function gp_caikuang_kunshankuangyu:OnProcOver(selfId, targetId)
    local ABilityID = self:GetItemBoxRequireAbilityID(targetId)
    self:CallScriptFunction(define.ABILITYLOGIC_ID, "EnergyCostCaiJi", selfId, ABilityID, self.g_AbilityLevel)
    return 0
end

function gp_caikuang_kunshankuangyu:OpenCheck(selfId, AbilityId, AbilityLevel)
...
return gp_caikuang_kunshankuangyu
