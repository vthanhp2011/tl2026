...
local std_talent_141 = class("std_talent_141", base)
local DI_DamagesByValue_T = require "scene.skill.impact_logic.std_impact_003"
local DAMAGE_TYPE_RATE = DI_DamagesByValue_T.enum_DAMAGE_TYPE_RATE

function std_talent_141:is_specific_skill(skill_id)
    return skill_id == 481
end

function std_talent_141:get_refix_value(talent, level)
    local params = talent.params[level]
    return params[1]
end

function std_talent_141:on_damage_target(talent, level, id, _, reciver, damages, skill_id)
    if self:is_specific_skill(skill_id) then
        if reciver:impact_have_impact_of_specific_impact_id(509) then
