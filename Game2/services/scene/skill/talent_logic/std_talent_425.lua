...
local std_talent_425 = class("std_talent_425", base)
local DI_DamagesByValue_T = require "scene.skill.impact_logic.std_impact_003"
local DAMAGE_TYPE_RATE = DI_DamagesByValue_T.enum_DAMAGE_TYPE_RATE

function std_talent_425:get_refix_value(talent, level)
    local params = talent.params[level]
    return params[1]
end

function std_talent_425:on_damage_target(talent, level, id, sender, reciver, damages, skill_id)
    if sender:impact_have_impact_of_specific_impact_id(108) or sender:impact_have_impact_of_specific_impact_id(105) then
        if reciver:impact_get_first_impact_in_specific_collection(43) then
