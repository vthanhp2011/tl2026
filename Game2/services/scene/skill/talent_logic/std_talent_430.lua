...
local std_talent_430 = class("std_talent_430", base)
local DI_DamagesByValue_T = require "scene.skill.impact_logic.std_impact_003"
local DAMAGE_TYPE_RATE = DI_DamagesByValue_T.enum_DAMAGE_TYPE_RATE

function std_talent_430:is_specific_skill(skill_id)
    return skill_id == 283
end

function std_talent_430:get_refix_value(talent, level)
    local params = talent.params[level]
    return params[1]
end

function std_talent_430:on_damage_target(talent, level, id, sender, reciver, damages, skill_id)
    if self:is_specific_skill(skill_id) then
        local impact = sender:impact_get_first_impact_of_specific_impact_id(519)
        if impact and impact:get_caster_obj_id() == sender:get_obj_id()  then
