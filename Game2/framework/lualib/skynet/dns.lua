...
local TIMEOUT = 1 * 100	-- 30 seconds
local MIN_TTL = 5 * 60 -- 5 minute
...
local function resolve(content)
	...
	ttl = ttl < MIN_TTL and MIN_TTL or ttl
	if answers_ipv4 then
		CACHE[QTYPE.A][question.name] = { answers = answers_ipv4, ttl = skynet.now() + ttl * 100, last = skynet.now() }
	end

	if answers_ipv6 then
		CACHE[QTYPE.AAAA][question.name] = { answers = answers_ipv6, ttl = skynet.now() + ttl * 100, last = skynet.now() }
	end
...
local DNS_CACHE_DROP_RETIRE = 10 * 60 * 100
local function touch_server()
	...
	local function drop_cache()
		for name, answers in pairs(CACHE[QTYPE.A]) do
			answers.last = answers.last or 0
			local diff = skynet.now() - answers.last
			if diff > DNS_CACHE_DROP_RETIRE then
				CACHE[QTYPE.A][name] = nil
			end
		end
		for name, answers in pairs(CACHE[QTYPE.AAAA]) do
			answers.last = answers.last or 0
			local diff = skynet.now() - answers.last
			if diff > DNS_CACHE_DROP_RETIRE then
				CACHE[QTYPE.AAAA][name] = nil
			end
		end
		skynet.timeout( 2 * DNS_CACHE_DROP_RETIRE, drop_cache)
	end
	skynet.timeout( 2 * DNS_SERVER_RETIRE, check_alive)
	skynet.timeout( 2 * DNS_CACHE_DROP_RETIRE, drop_cache)
end
