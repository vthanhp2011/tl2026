...
checkfilestate(struct logger *inst){
	...
	if(!issameday(nowtime, inst->createtime) || inst->filesize > MAXFILESIZE){
		...
		timestring(nowtime, fmt, tmp);
		snprintf(fullpath, SIZEPATH, "%s/%s", inst->path, tmp);
		if (access(inst->path, 0) != 0)
			mkdir(inst->path, 0777);
		if (access(fullpath, 0) != 0)
			mkdir(fullpath, 0777);
		char fullname[SIZEFULLNAME];
		snprintf(fullname, SIZEFULLNAME, "%s/%s", fullpath, newfilename);
...
logger_cb(struct skynet_context * context, void *ud, int type, int session, uint32_t source, const void * msg, size_t sz) {
	...
	case PTYPE_SYSTEM:
		...
		if (msg){
			char prefix[sz];
			char path[sz];
			int n = sscanf(msg, "%s %s", prefix, path);
			if (n == 2){
				inst->prefix = skynet_malloc(strlen(prefix)+1);
				inst->path = skynet_malloc(strlen(path)+1);
				strcpy(inst->prefix, prefix);
...
logger_init(struct logger * inst, struct skynet_context *ctx, const char * parm) {
	...
	if (parm) {
		...
		char prefix[sz];
		char path[sz];
		int n = sscanf(parm, "%s %s", prefix, path);
		if (n == 2){
			inst->prefix = skynet_malloc(strlen(prefix)+1);
			inst->path = skynet_malloc(strlen(path)+1);
			strcpy(inst->prefix, prefix);
			...
			printf("Invalid logger parm %s",parm);
		}
	} else {
		inst->handle = stdout;
	}
