...
callbackmessage(void *ud, uint32_t watcher, uint32_t marker) {
    struct aoi_space_plus *sp = ud;
    struct skynet_context* ctx = sp->ctx;
    size_t sz = getnumbercount(watcher) + getnumbercount(marker) + strlen("aoicallback") + 30;

    char *msg = skynet_malloc(sz);
    if(msg!=NULL){
        memset(msg, 0, sz);
        //printf("callbackmessage watcher = %d marker = %d sz = %ld\n", watcher, marker, sz);
        snprintf(msg, sz, "aoicallback-%d-%d", watcher, marker);
        skynet_send(ctx, 0, sp->dest, PTYPE_TEXT | PTYPE_TAG_DONTCOPY, 0, (void *)msg, sz);
    }
}
...
static void
_ctrl(struct skynet_context *ctx, struct aoi_space_plus *sp, const void *msg, int sz) {
    char tmp[sz + 1];
    memcpy(tmp, msg, sz);
    tmp[sz] = '\0';
    ...
    if (memcmp(command, "update", i) == 0) {
        ...
        aoi_update(sp->space, id, mode, pos,ctx);
        return;
    }
    if (memcmp(command, "message", i) == 0)
    {
        aoi_message(sp->space, callbackmessage, sp);
        return;
    }
    if(memcmp(command, "register", i) == 0)
    {
        char *text = tmp;
        strsep(&text, " ");
        char* deststr = strsep(&text, " ");
        sp->dest = strtol(deststr, NULL, 10);
    }
