...
local skill_292 = class("skill_292", base)

function skill_292:get_impact(skill_info)
    local impacts = skill_info:get_activate_once_impacts()
    return impacts[1] or define.INVAILD_ID
end

function skill_292:get_impact_give_self(skill_info)
    local impacts = skill_info:get_activate_once_impacts()
    return impacts.self or define.INVAILD_ID
end

function skill_292:get_tired_impact(skill_info)
    local descriptor = skill_info:get_descriptor()
    return descriptor["疲劳buff的ID"]
end

function skill_292:effect_on_unit_once(obj_me, obj_tar, is_critical)
    ...
    if value ~= define.INVAILD_ID then
        print("skill_292:effect_on_unit_once impact obj_id =", obj_me:get_obj_id(), ";logic_id =", value)
    ...
    value = self:get_impact_give_self(skill_info)
    if value ~= define.INVAILD_ID then
        print("skill_292:effect_on_unit_once impact obj_id =", obj_me:get_obj_id(), ";logic_id =", value)
        local imp = impact.new()
        imp:clean_up()
