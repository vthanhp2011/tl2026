...
local skill_715 = class("skill_715", base)
local DAMAGE_TYPE_RATE = DI_DamagesByValue_T.enum_DAMAGE_TYPE_RATE
function skill_715:ctor()

end
function skill_715:get_basic_attack_id(skill_info)
	local descriptor = skill_info:get_descriptor()
	return descriptor["普攻id"] or define.INVAILD_ID
end
function skill_715:get_lost_soul_state_ids(skill_info)
	local descriptor = skill_info:get_descriptor()
	return descriptor["丧魂buff集合"] or define.INVAILD_ID
end
function skill_715:get_lose_soul_percentage(skill_info)
	local descriptor = skill_info:get_descriptor()
	return descriptor["丧魂百分比"] or define.INVAILD_ID
end
function skill_715:get_second_lose_soul_percentage(skill_info)
	local descriptor = skill_info:get_descriptor()
	return descriptor["2丧魂百分比"] or define.INVAILD_ID
end
function skill_715:get_third_lose_soul_percentage(skill_info)
	local descriptor = skill_info:get_descriptor()
	return descriptor["3丧魂百分比"] or define.INVAILD_ID
end
function skill_715:get_kill_and_blind(skill_info)
	local descriptor = skill_info:get_descriptor()
	return descriptor["斩杀失明"] or define.INVAILD_ID
end
function skill_715:specific_condition_check(obj_me)
...
function skill_715:effect_on_unit_once(obj_me, obj_tar, is_critical)
    local skill_info = obj_me:get_skill_info()
    local params = obj_me:get_targeting_and_depleting_params()
    local value
