...
local std_talent_578 = class("std_talent_578", base)
local DI_DamagesByValue_T = require "scene.skill.impact_logic.std_impact_003"
local DAMAGE_TYPE_RATE = DI_DamagesByValue_T.enum_DAMAGE_TYPE_RATE
function std_talent_578:get_refix_value(talent, level)
    local params = talent.params[level]
    return params[1]
end

function std_talent_578:get_refix_value_2(talent, level)
    local params = talent.params[level]
    return params[2]
end

function std_talent_578:on_damage_target(talent, level, id, sender, reciver, damages, skill_id, imp)
    if reciver:impact_have_impact_of_specific_impact_id(184) or reciver:impact_have_impact_of_specific_impact_id(231)  then
        local percent = self:get_refix_value(talent, level)
        if imp and imp:is_critical_hit() then
            percent = self:get_refix_value_2(talent, level)
        end
        local percent = self:get_refix_value(talent, level)
