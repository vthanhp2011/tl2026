...
local std_talent_293 = class("std_talent_293", base)
local DI_DamagesByValue_T = require "scene.skill.impact_logic.std_impact_003"
local DAMAGE_TYPE_RATE = DI_DamagesByValue_T.enum_DAMAGE_TYPE_RATE


function std_talent_293:get_refix_value(talent, level)
    local params = talent.params[level]
    return params[1]
end

function std_talent_293:on_damage_target(talent, level, id, sender, reciver, damages, skill_id)
    if reciver:impact_have_impact_of_specific_impact_id(144) then
