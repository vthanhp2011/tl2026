...
local std_talent_625 = class("std_talent_625", base)
local DI_DamagesByValue_T = require "scene.skill.impact_logic.std_impact_003"
local DAMAGE_TYPE_RATE = DI_DamagesByValue_T.enum_DAMAGE_TYPE_RATE

function std_talent_625:is_specific_skill(skill_id)
    return skill_id == 791
end

function std_talent_625:get_refix_value(talent, level)
    local params = talent.params[level]
    return params[1]
end

function std_talent_625:on_damage_target(talent, level, id, sender, reciver, damages, skill_id)
