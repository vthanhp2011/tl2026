...
local odali_chargetransfer = class("odali_chargetransfer", script_base)
odali_chargetransfer.script_id = 400958
odali_chargetransfer.g_name = "车传志"
odali_chargetransfer.g_transfer_target = {
...
odali_chargetransfer.g_transfer_cost = 5000
function odali_chargetransfer:OnDefaultEvent(selfId, targetId)
    self:UpdateEventList(selfId, targetId)
end

function odali_chargetransfer:UpdateEventList(selfId, targetId)
...
function odali_chargetransfer:OnEventRequest(selfId, targetId, arg, index)
    if self:GetLevel(selfId) < 10 then
        self:BeginEvent(self.script_id)
        self:AddText("  你级别太低，10级以上才能传送去别的城市。")
        self:EndEvent()
        self:DispatchEventList(selfId, targetId)
        return
    end
    ...
    elseif id == 100 then
        ...
        if (nMoneyJZ + nMoney) >= self.g_transfer_cost then
            if self:LuaFnCostMoneyWithPriority(selfId, self.g_transfer_cost) == -1 then
                self:BeginEvent(self.script_id)
                self:AddText("收费失败！")
                self:EndEvent()
...
function odali_chargetransfer:NotifyFailTips(selfId, targetId, Tip)
    self:BeginEvent(self.script_id)
    self:AddText(Tip)
    self:EndEvent()
    self:DispatchEventList(selfId, targetId)
