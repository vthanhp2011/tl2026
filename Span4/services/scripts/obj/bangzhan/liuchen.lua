...
local liuchen = class("liuchen", script_base)
liuchen.script_id = 402299
liuchen.g_eventList = {701603}

liuchen.g_healScriptId = 000064
function liuchen:UpdateEventList(selfId, targetId)
...
function liuchen:OnDefaultEvent(selfId, targetId)
    self:UpdateEventList(selfId, targetId)
end

function liuchen:OnEventRequest(selfId, targetId, arg, index)
...
function liuchen:OnMissionAccept(selfId, targetId, missionScriptId)
    for i, findId in pairs(self.g_eventList) do
...
function liuchen:OnMissionRefuse(selfId, targetId, missionScriptId)
    for i, findId in pairs(self.g_eventList) do
        if missionScriptId == findId then
            self:UpdateEventList(selfId, targetId)
            return
...
function liuchen:OnMissionContinue(selfId, targetId, missionScriptId)
    for i, findId in pairs(self.g_eventList) do
        if missionScriptId == findId then
...
function liuchen:OnMissionSubmit(selfId, targetId, missionScriptId, selectRadioId)
    for i, findId in pairs(self.g_eventList) do
        if missionScriptId == findId then
...
function liuchen:OnDie(selfId, killerId)
end
