...
local gp_diaoyu_qingyu = class("gp_diaoyu_qingyu", script_base)
gp_diaoyu_qingyu.g_MainItemId = 20102013
gp_diaoyu_qingyu.g_AbilityId = 9
gp_diaoyu_qingyu.g_AbilityLevel = 1
function gp_diaoyu_qingyu:OnCreate(growPointType, x, y)
    local ItemCount = 0
    self:ItemBoxEnterScene(x, y, growPointType, define.QUALITY_MUST_BE_CHANGE, ItemCount)
end

function gp_diaoyu_qingyu:OnOpen(selfId, targetId)
...
function gp_diaoyu_qingyu:OnProcOver(selfId, targetId)
    local ret_1 = self:TryRecieveItem(selfId, self.g_MainItemId, define.QUALITY_MUST_BE_CHANGE)
    if ret_1 then
        self:Msg2Player(selfId, "你钓到一条青鱼。", define.ENUM_CHAT_TYPE.MSG2PLAYER_PARA)
        local ABilityID = self:GetItemBoxRequireAbilityID(targetId)
