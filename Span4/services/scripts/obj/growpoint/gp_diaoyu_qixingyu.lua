...
local gp_diaoyu_qixingyu = class("gp_diaoyu_qixingyu", script_base)
gp_diaoyu_qixingyu.g_MainItemId = 20102041
gp_diaoyu_qixingyu.g_AbilityId = 9
gp_diaoyu_qixingyu.g_AbilityLevel = 10
function gp_diaoyu_qixingyu:OnCreate(growPointType, x, y)
    local ItemCount = 0
    self:ItemBoxEnterScene(x, y, growPointType, define.QUALITY_MUST_BE_CHANGE, ItemCount)
end

function gp_diaoyu_qixingyu:OnOpen(selfId, targetId)
...
function gp_diaoyu_qixingyu:OnProcOver(selfId, targetId)
    local ret_1 = self:TryRecieveItem(selfId, self.g_MainItemId, define.QUALITY_MUST_BE_CHANGE)
    if ret_1 then
        self:Msg2Player(selfId, "你钓到一条七星鱼。", define.ENUM_CHAT_TYPE.MSG2PLAYER_PARA)
        local ABilityID = self:GetItemBoxRequireAbilityID(targetId)
