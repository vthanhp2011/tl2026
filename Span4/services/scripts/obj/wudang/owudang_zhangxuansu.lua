...
local owudang_zhangxuansu = class("owudang_zhangxuansu", script_base)
owudang_zhangxuansu.script_id = 12006
local MenPaiPet =
{
...
function owudang_zhangxuansu:OnDefaultEvent(selfId, targetId)
	local menpai = self:GetMenPai(selfId)
    self:BeginEvent(self.script_id)
        self:AddText("  贫道乃张玄素是也，武当山上的道家香火，乃是贫道十五年前开创。")
        if menpai == 9 then
...
function owudang_zhangxuansu:OnEventRequest(selfId, targetId, arg, index)
	...
	if index == 0 then
		...
		if nMenPai == 3 then
			self:MsgBox(selfId,targetId,"#{event_wudang_0001}")
			return
		end
		self:BeginEvent(self.script_id)
		self:AddText(" #{MenpaiInfo_003}")
		self:AddNumText("我确定要拜入武当派", 6,3)
		self:AddNumText("我暂时还不想拜入门派", 8,1000)
		self:EndEvent()
		self:DispatchEventList(selfId, targetId)
	elseif index == 1 then
		self:MsgBox(selfId, targetId, " #{OBJ_wudang_0003}")
	elseif index == 6 then
		self:MsgBox(selfId, targetId, " #{MPJN_090728_08}")
	elseif index == 3 then
		...
		if self:GetMenPai(selfId) == 9 then
			self:LuaFnJoinMenpai(selfId,3)
			self:LuaFnSetXinFaLevel(selfId,19,10)
			self:LuaFnSetXinFaLevel(selfId,20,1)
			self:LuaFnSetXinFaLevel(selfId,21,1)
			self:LuaFnSetXinFaLevel(selfId,22,1)
			self:LuaFnSetXinFaLevel(selfId,23,10)
			self:LuaFnSetXinFaLevel(selfId,24,1)
			self:AddItemListToHuman(selfId)
			self:notify_tips(selfId, "你已经加入武当派！")
			self:notify_tips(selfId, "你获得了" .. self:GetItemName(10124004) .. "。")
	...
	elseif index == 100 then
		...
		self:BeginAddItem()
		self:AddItem(10432010, 1,true)
		self:AddItem(10443410, 1,true)
		local ret = self:EndAddItem(selfId)
		if not ret then
			self:MsgBox(selfId, targetId, "  你的背包空间不够了，整理后再来找我。")
			return
		...
		self:AddItemListToHuman(selfId)
		self:notify_tips(selfId, "你获得了" .. self:GetItemName(10432010) .. "。")
		self:notify_tips(selfId, "你获得了" .. self:GetItemName(10443410) .. "。")
	...
	elseif index == 102 then
		...
		if self:GetItemCount(selfId, 10124004) > 0 then
...
function owudang_zhangxuansu:MsgBox(selfId, targetId, msg)
	self:BeginEvent(self.script_id)
	self:AddText(msg)
	self:EndEvent()
	self:DispatchEventList(selfId, targetId)
end
return owudang_zhangxuansu