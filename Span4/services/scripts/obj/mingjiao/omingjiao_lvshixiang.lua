...
local omingjiao_lvshixiang = class("omingjiao_lvshixiang", script_base)
omingjiao_lvshixiang.script_id = 11006
local MenPaiPet =
{
...
function omingjiao_lvshixiang:OnDefaultEvent(selfId, targetId)
	local menpai = self:GetMenPai(selfId)
    self:BeginEvent(self.script_id)
        self:AddText("  洒家是明教右使吕师襄，若要加入明教，便来找洒家！若要喝酒，也来找洒家！")
        if menpai == 9 then
...
function omingjiao_lvshixiang:OnEventRequest(selfId, targetId, arg, index)
	...
	if index == 0 then
		...
		if nMenPai == 1 then
			self:MsgBox(selfId,targetId,"#{event_mingjiao_0001}")
			return
		end
		self:BeginEvent(self.script_id)
		self:AddText(" #{MenpaiInfo_001}")
		self:AddNumText("我确定要拜入明教", 6,3)
		self:AddNumText("我暂时还不想拜入门派", 8,1000)
		self:EndEvent()
		self:DispatchEventList(selfId, targetId)
	elseif index == 1 then
		self:MsgBox(selfId, targetId, " #{OBJ_mingjiao_0002}")
	elseif index == 6 then
		self:MsgBox(selfId, targetId, " #{MPJN_090728_09}")
	elseif index == 3 then
		...
		if self:GetMenPai(selfId) == 9 then
			self:LuaFnJoinMenpai(selfId, 1)
			self:LuaFnSetXinFaLevel(selfId,7,10)
			self:LuaFnSetXinFaLevel(selfId,8,1)
			self:LuaFnSetXinFaLevel(selfId,9,1)
			self:LuaFnSetXinFaLevel(selfId,10,1)
			self:LuaFnSetXinFaLevel(selfId,11,10)
			self:LuaFnSetXinFaLevel(selfId,12,1)
			self:AddItemListToHuman(selfId)
			self:notify_tips(selfId, "你已经加入明教！")
			self:notify_tips(selfId, "你获得了" .. self:GetItemName(10124001) .. "。")
	...
	elseif index == 100 then
		...
		self:BeginAddItem()
		self:AddItem(10431110, 1,true)
		self:AddItem(10443110, 1,true)
		local ret = self:EndAddItem(selfId)
		if not ret then
			self:MsgBox(selfId, targetId, "  你的背包空间不够了，整理后再来找我。")
			return
		...
		self:AddItemListToHuman(selfId)
		self:notify_tips(selfId, "你获得了" .. self:GetItemName(10431110) .. "。")
		self:notify_tips(selfId, "你获得了" .. self:GetItemName(10443110) .. "。")
	...
	elseif index == 102 then
		...
		if self:GetItemCount(selfId, 10124001) > 0 then
...
function omingjiao_lvshixiang:MsgBox(selfId, targetId, msg)
	self:BeginEvent(self.script_id)
	self:AddText(msg)
	self:EndEvent()
	self:DispatchEventList(selfId, targetId)
end
return omingjiao_lvshixiang