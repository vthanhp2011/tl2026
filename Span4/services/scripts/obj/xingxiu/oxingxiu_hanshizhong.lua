...
local oxingxiu_hanshizhong = class("oxingxiu_hanshizhong", script_base)
oxingxiu_hanshizhong.script_id = 16001
local MenPaiPet =
{
...
function oxingxiu_hanshizhong:OnDefaultEvent(selfId, targetId)
	local menpai = self:GetMenPai(selfId)
    self:BeginEvent(self.script_id)
        self:AddText("  我是星宿派的现任大师兄韩世忠。")
        if menpai == 9 then
...
function oxingxiu_hanshizhong:OnEventRequest(selfId, targetId, arg, index)
	...
	if index == 0 then
		...
		if nMenPai == 5 then
			self:MsgBox(selfId,targetId,"#{event_xingxiu_0001}")
			return
		end
		self:BeginEvent(self.script_id)
		self:AddText(" #{MenpaiInfo_005}")
		self:AddNumText("我确定要拜入星宿派", 6,3)
		self:AddNumText("我暂时还不想拜入门派", 8,1000)
		self:EndEvent()
		self:DispatchEventList(selfId, targetId)
	elseif index == 1 then
		self:MsgBox(selfId, targetId, " #{OBJ_xingxiu_0003}")
	elseif index == 6 then
		self:MsgBox(selfId, targetId, " #{MPJN_090728_03}")
	elseif index == 3 then
		...
		if self:GetMenPai(selfId) == 9 then
			self:LuaFnJoinMenpai(selfId,5)
			self:LuaFnSetXinFaLevel(selfId,31,10)
			self:LuaFnSetXinFaLevel(selfId,32,1)
			self:LuaFnSetXinFaLevel(selfId,33,1)
			self:LuaFnSetXinFaLevel(selfId,34,1)
			self:LuaFnSetXinFaLevel(selfId,35,10)
			self:LuaFnSetXinFaLevel(selfId,36,1)
			self:AddItemListToHuman(selfId)
			self:notify_tips(selfId, "你已经加入星宿派！")
			self:notify_tips(selfId, "你获得了" .. self:GetItemName(10124005) .. "。")
	...
	elseif index == 100 then
		...
		self:BeginAddItem()
		self:AddItem(10433210, 1,true)
		self:AddItem(10443510, 1,true)
		local ret = self:EndAddItem(selfId)
		if not ret then
			self:MsgBox(selfId, targetId, "  你的背包空间不够了，整理后再来找我。")
			return
		...
		self:AddItemListToHuman(selfId)
		self:notify_tips(selfId, "你获得了" .. self:GetItemName(10433210) .. "。")
		self:notify_tips(selfId, "你获得了" .. self:GetItemName(10443510) .. "。")
	...
	elseif index == 102 then
		...
		if self:GetItemCount(selfId, 10124005) > 0 then
...
function oxingxiu_hanshizhong:MsgBox(selfId, targetId, msg)
	self:BeginEvent(self.script_id)
	self:AddText(msg)
	self:EndEvent()
	self:DispatchEventList(selfId, targetId)
end
return oxingxiu_hanshizhong