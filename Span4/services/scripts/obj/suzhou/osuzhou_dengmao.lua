...
local osuzhou_dengmao = class("osuzhou_dengmao", script_base)
osuzhou_dengmao.script_id = 001081
osuzhou_dengmao.g_mpInfo = {}

osuzhou_dengmao.g_mpInfo[0] = {"星宿", 16, 96, 152, define.MENPAI_ATTRIBUTE.MATTRIBUTE_XINGXIU}

osuzhou_dengmao.g_mpInfo[1] = {"逍遥", 14, 67, 145, define.MENPAI_ATTRIBUTE.MATTRIBUTE_XIAOYAO}

osuzhou_dengmao.g_mpInfo[2] = {"少林", 9, 96, 127, define.MENPAI_ATTRIBUTE.MATTRIBUTE_SHAOLIN}

osuzhou_dengmao.g_mpInfo[3] = {"天山", 17, 95, 120, define.MENPAI_ATTRIBUTE.MATTRIBUTE_TIANSHAN}

osuzhou_dengmao.g_mpInfo[4] = {"天龙", 13, 96, 120, define.MENPAI_ATTRIBUTE.MATTRIBUTE_DALI}

osuzhou_dengmao.g_mpInfo[5] = {"峨嵋", 15, 89, 139, define.MENPAI_ATTRIBUTE.MATTRIBUTE_EMEI}

osuzhou_dengmao.g_mpInfo[6] = {"武当", 12, 103, 140, define.MENPAI_ATTRIBUTE.MATTRIBUTE_WUDANG}

osuzhou_dengmao.g_mpInfo[7] = {"明教", 11, 98, 167, define.MENPAI_ATTRIBUTE.MATTRIBUTE_MINGJIAO}

osuzhou_dengmao.g_mpInfo[8] = {"丐帮", 10, 91, 116, define.MENPAI_ATTRIBUTE.MATTRIBUTE_GAIBANG}

osuzhou_dengmao.g_Yinpiao = 40002000
osuzhou_dengmao.g_Impact_NotTransportList = {5929, 5944}

osuzhou_dengmao.g_TalkInfo_NotTransportList = {"#{GodFire_Info_062}", "#{XSHCD_20080418_099}"}

function osuzhou_dengmao:OnDefaultEvent(selfId, targetId)
...
function osuzhou_dengmao:OnEventRequest(selfId, targetId, arg, index)
...
function osuzhou_dengmao:GotoShuHeGuZhen(selfId, targetId)
    self:CallScriptFunction((400900), "TransferFunc", selfId, 420, 200, 211, 20)
    return
end

function osuzhou_dengmao:GetMPInfo(mpID)
    local mp
    local i = 0
    for i, mp in pairs(self.g_mpInfo) do
        if mp[5] == mpID then
...
function osuzhou_dengmao:MsgBox(selfId, targetId, msg)
    self:BeginEvent(self.script_id)
    self:AddText(msg)
    self:EndEvent()
    self:DispatchEventList(selfId, targetId)
