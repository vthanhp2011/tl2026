...
ExchangeTank.g_A_SecondTankBuff = 1
...
function ExchangeTank:OnDefaultEvent(selfId, targetId, index)
    ...
    if numText == 1000 or numText == 2000 or numText == 3000 or numText == 4000 or
        ...
        if (TankManFirst ~= 0 and
            self:CallScriptFunction(self.g_BangzhanScriptId, "HaveTankBuff", TankManFirst) == self.g_AttrBuff[index + base]) 
            or (TankManSecond ~= 0 and self:CallScriptFunction(self.g_BangzhanScriptId, "HaveTankBuff", TankManSecond) == self.g_AttrBuff[index + base]) or
            (self:FindWildTank(index + base) ~= 0) then
            self:AddText("  想让你的" .. self.g_TankName[index] .. "#{BHXZ_081103_54}")
            for i = 1, #(self.g_TankName) do
                if index ~= i then
            ...
            if TankManFirst ~= 0 and
                ...
                if (TankManSecond ~= 0 and
                    ...
                    if num < self.g_WantLingShiNum then
                        self:NotifyFailBox(selfId, targetId, "#{BHXZ_081103_47}" .. self.g_LingShi[hundred] .. "#{BHXZ_081103_56}" .. num)
                    else
                        num = num - self.g_WantLingShiNum
                        self:SetGuildIntNum(guildid, beginindex + hundred - 1, num)
                        self:LuaFnSetCopySceneData_Param(self.g_A_FirstTankManSelfID + intbase, 0)
                        self:LuaFnSetCopySceneData_Param(self.g_A_FirstTankBuff + intbase, 0)
                        self:LuaFnSetCopySceneData_Param(self.g_A_FirstTankPos + intbase, 0)
                        self:LuaFnCancelSpecificImpact(TankManFirst, self.g_AttrBuff[thousand + base])
                        local selfIdindex = self:CallScriptFunction(600051, "FindTankManIndex",isguildA, hundred + base)
                    ...
                    if (TankManFirst ~= 0 and
                        ...
                        if num < self.g_WantLingShiNum then
                            ...
                            local addpoint =
                                self:GetGuildWarPoint(self.g_GuildPoint_ExchangeTank)
                            if isguildA == 1 then
                                self:CallScriptFunction(self.g_BangzhanScriptId,  "AddAGuildPoint", 0, guildid, addpoint)
                            else
                                self:CallScriptFunction(self.g_BangzhanScriptId, "AddBGuildPoint", 0, guildid, addpoint)
                            end
                            self:NotifyFailBox(selfId, targetId, "#{BHXZ_081103_111}")
                            self:Msg2Player(selfId, "#{BHXZ_081103_138}" .. self.g_TankName[thousand] .. "转换成了" ..  self.g_TankName[hundred] .. "。", define.ENUM_CHAT_TYPE.MSG2PLAYER_PARA)
                        end
                    end
                else
                    self:NotifyFailBox(selfId, targetId, "#{BHXZ_081103_42}" .. self.g_TankName[thousand] .. "。")
                end
            end
        elseif ten == 2 then
            self:BeginUICommand()
    ...
    if numText > 6000 and numText < 11000 then
        ...
        if hundred == 1 then
            ...
            if TankManFirst ~= 0 and
                ...
                self:UICommand_AddStr("    #{BHXZ_081103_152}" .. self.g_TankName[thousand] ..  "#{BHXZ_081103_153}")
                self:EndUICommand()
                self:DispatchUICommand(selfId, 24)
            elseif TankManSecond ~= 0 and
                self:CallScriptFunction(self.g_BangzhanScriptId, "HaveTankBuff", TankManSecond) == self.g_AttrBuff[thousand + base] then
                self:BeginUICommand()
                self:UICommand_AddInt(self.script_id)
                self:UICommand_AddInt(targetId)
                self:UICommand_AddInt(thousand + base)
...
function ExchangeTank:ChangeOK(selfId, targetId, tanktype)
    ...
    if TankManFirst ~= 0 and
        self:CallScriptFunction(self.g_BangzhanScriptId, "HaveTankBuff", TankManFirst) == self.g_AttrBuff[tanktype] then
        for i = 0, 4 do
            self:SetGuildIntNum(guildid, beginindex + i, countarray[i + 1] - 20)
        end
        self:SetPos(TankManFirst, self.g_DefaultTankPos[1 + intbase][1], self.g_DefaultTankPos[1 + intbase][2])
        if 1 <= tanktype and tanktype <= 5 then
            self:NotifyFailBox(selfId, targetId, "   " .. self.g_TankName[tanktype] .. "#{BHXZ_081103_154}")
            self:Msg2Player(selfId, "#{BHXZ_081103_138}" .. self.g_TankName[tanktype] .. "#{BHXZ_081103_139}", define.ENUM_CHAT_TYPE.MSG2PLAYER_PARA)
        elseif 6 <= tanktype and tanktype <= 10 then
            self:NotifyFailBox(selfId, targetId, "   " .. self.g_TankName[tanktype - 5] .. "#{BHXZ_081103_154}")
            self:Msg2Player(selfId, "#{BHXZ_081103_138}" .. self.g_TankName[tanktype - 5] .. "#{BHXZ_081103_139}", define.ENUM_CHAT_TYPE.MSG2PLAYER_PARA)
        end

    elseif TankManSecond ~= 0 and
        self:CallScriptFunction(self.g_BangzhanScriptId, "HaveTankBuff", TankManSecond) == self.g_AttrBuff[tanktype] then
        for i = 0, 4 do
            self:SetGuildIntNum(guildid, beginindex + i, countarray[i + 1] - 20)
        end
        self:SetPos(TankManFirst, self.g_DefaultTankPos[1 + intbase][1],self.g_DefaultTankPos[1 + intbase][2])
        ...
        if tankobjID ~= 0 then
            ...
            self:SetPos(tankobjID, self.g_DefaultTankPos[1 + intbase][1], self.g_DefaultTankPos[1 + intbase][2])
            if 1 <= tanktype and tanktype <= 5 then
                self:NotifyFailBox(selfId, targetId, "   " .. self.g_TankName[tanktype] .. "#{BHXZ_081103_154}")
                self:Msg2Player(selfId, "#{BHXZ_081103_138}" ..self.g_TankName[tanktype] .."#{BHXZ_081103_139}", define.ENUM_CHAT_TYPE.MSG2PLAYER_PARA)
            elseif 6 <= tanktype and tanktype <= 10 then
            ...
            if 1 <= tanktype and tanktype <= 5 then
                self:NotifyFailBox(selfId, targetId, "#{BHXZ_081103_42}" .. self.g_TankName[tanktype] .. "。")
            elseif 6 <= tanktype and tanktype <= 10 then
                self:NotifyFailBox(selfId, targetId, "#{BHXZ_081103_42}" ..self.g_TankName[tanktype - 5] .. "。")
            end
        end
    end
end
