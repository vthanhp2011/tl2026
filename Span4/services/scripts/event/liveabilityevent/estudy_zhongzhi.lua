...
local estudy_zhongzhi = class("estudy_zhongzhi", script_base)
estudy_zhongzhi.script_id = 713511
estudy_zhongzhi.g_MessageStudy = "如果你达到%d级并且肯花费#{_EXCHG%d}就可以学会种植技能。你决定学习么？"
estudy_zhongzhi.g_AbilityID = define.ABILITY_ZHONGZHI
estudy_zhongzhi.g_AbilityName = "种植"
function estudy_zhongzhi:OnDefaultEvent(selfId, targetId, ButtomNum, g_Npc_ScriptId, bid)
    ...
    if ButtomNum == 0 then
        local ret, demandMoney, demandExp, limitAbilityExp, limitAbilityExpShow, currentLevelAbilityExpTop, limitLevel, extraMoney, extraExp = self:LuaFnGetAbilityLevelUpConfig2(define.ABILITY_ZHONGZHI, 1)
        if ret then
            self:BeginEvent(self.script_id)
...
function estudy_zhongzhi:OnEnumerate(caller, selfId, targetId, bid)
    if bid then
    ...
    local ret = self:LuaFnGetAbilityLevelUpConfig(define.ABILITY_ZHONGZHI, 1)
    if ret then
        caller:AddNumTextWithTarget(self.script_id, "学习" .. self.g_AbilityName .. "技能", 12, 0)
    end
    return
...
function estudy_zhongzhi:CheckAccept()

end

function estudy_zhongzhi:OnAccept()

end

function estudy_zhongzhi:StudyInCity(selfId, targetId, ButtomNum, g_Npc_ScriptId, bid)
    if bid then
        if 0 == ButtomNum then
