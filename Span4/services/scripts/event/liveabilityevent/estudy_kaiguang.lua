...
local estudy_kaiguang = class("estudy_kaiguang", script_base)
estudy_kaiguang.script_id = 713512
estudy_kaiguang.g_MessageStudy = "如果你达到%d级并且肯花费#{_EXCHG%d}就可以学会开光技能。你决定学习么？"
estudy_kaiguang.g_AbilityID = define.ABILITY_KAIGUANG
estudy_kaiguang.g_AbilityName = "开光"
function estudy_kaiguang:OnDefaultEvent(selfId, targetId, ButtomNum, g_Npc_ScriptId)
    ...
    if ButtomNum == 0 then
        local ret, demandMoney, demandExp, limitAbilityExp, limitAbilityExpShow, currentLevelAbilityExpTop, limitLevel, extraMoney, extraExp = self:LuaFnGetAbilityLevelUpConfig2(define.ABILITY_KAIGUANG, 1)
        if ret then
            self:BeginEvent(self.script_id)
...
function estudy_kaiguang:OnEnumerate(caller, selfId, targetId, bid)
    caller:AddNumTextWithTarget(self.script_id, "学习" .. self.g_AbilityName .. "技能", 12, 0)
end

function estudy_kaiguang:CheckAccept()

end

function estudy_kaiguang:OnAccept()

end

return estudy_kaiguang
