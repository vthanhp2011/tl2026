...
local estudy_caiyao = class("estudy_caiyao", script_base)
estudy_caiyao.script_id = 713509
estudy_caiyao.g_MessageStudy = "如果你达到%d级并且肯花费#{_EXCHG%d}就可以学会采药技能。你决定学习么？"
estudy_caiyao.g_AbilityID = define.ABILITY_CAIYAO
estudy_caiyao.g_AbilityName = "采药"
function estudy_caiyao:OnDefaultEvent(selfId, targetId, ButtomNum, g_Npc_ScriptId, bid)
    ...
    if ButtomNum == 0 then
        local ret, demandMoney, demandExp, limitAbilityExp, limitAbilityExpShow, currentLevelAbilityExpTop, limitLevel, extraMoney, extraExp = self:LuaFnGetAbilityLevelUpConfig2(define.ABILITY_CAIYAO, 1)
        if ret then
            self:BeginEvent(self.script_id)
...
function estudy_caiyao:OnEnumerate(caller, selfId, targetId, bid)
    if bid then
    ...
    local ret = self:LuaFnGetAbilityLevelUpConfig(define.ABILITY_CAIYAO, 1)
    if ret then
        caller:AddNumTextWithTarget(self.script_id, "学习" .. self.g_AbilityName .. "技能", 12, 0)
    end
    return
...
function estudy_caiyao:CheckAccept()

end

function estudy_caiyao:OnAccept()

end

function estudy_caiyao:StudyInCity(selfId, targetId, ButtomNum, g_Npc_ScriptId, bid)
    if bid then
        if 0 == ButtomNum then
