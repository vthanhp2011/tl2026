...
function chuan_song:on_enter_area(scene, obj)
        ...
        if IsOpen ~= 0 then
            self:CallScriptFunction((400900), "TransferFunc", obj:get_obj_id(), nSceneInfo[IsOpen], 160, 278,10) --根据人数分配玩家所进入的场景。
            return
        else
            self:notify_tips(obj:get_obj_id(),"当前洛阳场景人数上限，请暂时前往其他地图。")
            return
        end
     self:CallScriptFunction((400900), "TransferFunc", obj:get_obj_id(), 0, 160, 278)
end
