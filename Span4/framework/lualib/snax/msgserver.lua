...
function server.start(conf, port, protocol)
	...
	local function do_auth(fd, message, addr)
		...
		local username, index, hmac
		if not (protocol == "ws" or protocol == "wss") then
			username, index, hmac = string.match(message, "([^:]*):([^:]*):([^:]*)")
		else
			username, index, hmac = string.match(message, "([^:]*):([^:]*)")
		end
		local u = user_online[username]
		local uid, subid, servername =  server.userid(username)
		skynet.logi("do_auth uid =", uid, ";subid =", subid, ";servername =", servername, ";index =", index, ";hmac =", hmac)
		if u == nil then
			skynet.loge("uid=", uid, ";subid=", subid, ";servernam=", servername, "404 User Not Found")
	...
	local function auth(fd, addr, msg,sz)
		print("wsmsgserver.lua 1,",fd,sz)
		local ok, result 
		if not (protocol == "ws" or protocol == "wss") then
			local message = netpack.tostring(msg, sz)
			ok, result = pcall(do_auth, fd, message, addr)
		else
			ok, result = pcall(do_auth, fd, msg, addr)
		end
	...
	local function request(fd, msg, sz)
		...
		if not (protocol == "ws" or protocol == "wss") then
			local message = netpack.tostring(msg, sz)
			ok, err = pcall(do_request, fd, message)
		else
			ok, err = pcall(do_request, fd, msg)
		end
	...
	return gateserver.start(handler, port, protocol)
end
