...
local function token_middleware()
    ...
    return function(c)
        ...
        if not cookie then
            skynet.loge("cookie is nil:", err1)
        end

        local current_token
        local token_data, err2 = cookie:get(token_key)
        if err2 then
            skynet.logw("cannot get token_data:", err2)
        else
            if token_data then
        ...
        skynet.logi("token.init. token_key:", token_key)
        ...
        c.token = {
            set = function (...)
                ...
                skynet.logi("token.set: ", value)

                if err or not ok then
                    return skynet.loge("token.set error:", err)
                end
            end,
            ...
            refresh = function()
                if token_data and token_data ~= "" then
                    ...
                    if err or not ok then
                        return skynet.loge("token.refresh error:", err)
                    end
                end
            end,
            ...
            destroy = function()
                ...
                if err or not ok then
                    skynet.loge("token.destroy error:", err)
                    return false
                end
        ...
        if refresh_cookie then
            ...
            if not ok then
                skynet.loge("refresh cookie error:", e)
            end
        end
