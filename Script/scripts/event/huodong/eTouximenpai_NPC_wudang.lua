...
local eTouximenpai_NPC_wudang                       = class("eTouximenpai_NPC_wudang", script_base)
--副本名称
eTouximenpai_NPC_wudang.g_CopySceneName             = "武当山"

eTouximenpai_NPC_wudang.g_ClientRes                 = 12
--任务号
eTouximenpai_NPC_wudang.g_MissionId                 = 1250
--上一个任务的ID
eTouximenpai_NPC_wudang.g_MissionIdPre              = 0
--目标NPC
eTouximenpai_NPC_wudang.g_Name                      = "偷袭门派"
--是否是精英任务
eTouximenpai_NPC_wudang.g_IfMissionElite            = 1
--任务等级
eTouximenpai_NPC_wudang.g_MissionLevel              = 10000
--任务归类
eTouximenpai_NPC_wudang.g_MissionKind               = 1
--任务文本描述
eTouximenpai_NPC_wudang.g_MissionName               = "偷袭门派"
--任务描述
eTouximenpai_NPC_wudang.g_MissionInfo               = "  "
--任务目标
eTouximenpai_NPC_wudang.g_MissionTarget             = "  杀死所有的怪物即可完成任务。"
--未完成任务的npc对话
eTouximenpai_NPC_wudang.g_ContinueInfo              = "  "
--完成任务npc说话的话
eTouximenpai_NPC_wudang.g_MissionComplete           = "  "
...
--**********************************以上是动态****************************
--角色Mission变量说明
eTouximenpai_NPC_wudang.g_Param_IsMissionOkFail     = 0 --0号：当前任务是否完成(0未完成；1完成)
eTouximenpai_NPC_wudang.g_Param_killmonstercount    = 1 --1号：杀死任务小怪的数量
eTouximenpai_NPC_wudang.g_Param_killbosscount       = 2 --2号：杀死任务boss怪的数量
eTouximenpai_NPC_wudang.g_Param_sceneid             = 3 --3号：当前副本任务的场景号
eTouximenpai_NPC_wudang.g_Param_teamid              = 4 --4号：接副本任务时候的队伍号
eTouximenpai_NPC_wudang.g_Param_time                = 5 --5号：完成副本所用时间(单位：秒)
--6号：具体副本事件脚本占用
--7号：具体副本事件脚本占用
--MisDescEnd
--************************************************************************

eTouximenpai_NPC_wudang.g_CopySceneType             = ScriptGlobal.FUBEN_TIANSHAN1 --副本类型，定义在ScriptGlobal.lua里面
eTouximenpai_NPC_wudang.g_LimitMembers              = 1                 --可以进副本的最小队伍人数
eTouximenpai_NPC_wudang.g_TickTime                  = 5                 --回调脚本的时钟时间（单位：秒/次）
eTouximenpai_NPC_wudang.g_LimitTotalHoldTime        = 360               --副本可以存活的时间（单位：次数）,如果此时间到了，则任务将会失败
eTouximenpai_NPC_wudang.g_LimitTimeSuccess          = 500               --副本时间限制（单位：次数），如果此时间到了，任务完成
eTouximenpai_NPC_wudang.g_CloseTick                 = 6                 --副本关闭前倒计时（单位：次数）
eTouximenpai_NPC_wudang.g_NoUserTime                = 300               --副本中没有人后可以继续保存的时间（单位：秒）
eTouximenpai_NPC_wudang.g_Fuben_X                   = 93                --进入副本的位置X
eTouximenpai_NPC_wudang.g_Fuben_Z                   = 182               --进入副本的位置Z
eTouximenpai_NPC_wudang.g_Back_X                    = 93                --源场景位置X
eTouximenpai_NPC_wudang.g_Back_Z                    = 182               --源场景位置Z
eTouximenpai_NPC_wudang.g_Totalkillmonstercount     = 30                --需要杀死monster数量
eTouximenpai_NPC_wudang.g_Totalkillbosscount        = 1                 --需要杀死Boss数量

--副本数据索引对照
eTouximenpai_NPC_wudang.g_keySD                     = {}
eTouximenpai_NPC_wudang.g_keySD["typ"]              = 0 --设置副本类型
eTouximenpai_NPC_wudang.g_keySD["spt"]              = 1 --设置副本场景事件脚本号
eTouximenpai_NPC_wudang.g_keySD["tim"]              = 2 --设置定时器调用次数
eTouximenpai_NPC_wudang.g_keySD["scn"]              = 3 --设置副本入口场景号, 初始化
eTouximenpai_NPC_wudang.g_keySD["cls"]              = 4 --设置副本关闭标志, 0开放，1关闭
eTouximenpai_NPC_wudang.g_keySD["dwn"]              = 5 --设置离开倒计时次数
eTouximenpai_NPC_wudang.g_keySD["tem"]              = 6 --保存队伍号
eTouximenpai_NPC_wudang.g_keySD["x"]                = 7 --人物在入口场景中的x位置
eTouximenpai_NPC_wudang.g_keySD["z"]                = 8 --人物在入口场景中的z位置
eTouximenpai_NPC_wudang.g_keySD["killedmonsternum"] = 9 --杀死喽啰的数量
eTouximenpai_NPC_wudang.g_keySD["killedbossnum"]    = 10 --杀死Boss的数量
eTouximenpai_NPC_wudang.g_keySD["mp"]               = 11 --记录当前副本的 门派

--接取任务的最低等级
eTouximenpai_NPC_wudang.g_minLevel                  = 20
--需要配置小怪物
eTouximenpai_NPC_wudang.g_typMonster                = 749 --水鬼探子
--门派id
eTouximenpai_NPC_wudang.g_MenPaiID                  = ScriptGlobal.MP_WUDANG

eTouximenpai_NPC_wudang.g_NianNum                   = 5
eTouximenpai_NPC_wudang.g_NianPos                   = {
	{ x = 75, y = 141 },
	{ x = 62, y = 103 },
	{ x = 94, y = 103 },
	{ x = 98, y = 63 },
...
--任务入口函数
--**********************************
function eTouximenpai_NPC_wudang:OnDefaultEvent(selfId, targetId)
...
--列举事件
--**********************************
function eTouximenpai_NPC_wudang:OnEnumerate(_, selfId, targetId)
	local MenPaiID = self:GetMenPai(selfId)
	if MenPaiID ~= self.g_MenPaiID then
		self:NotifyTip(selfId,"你不是武当派弟子，叫武当派弟子来。")
		return
	end
...
--检测接受条件
--**********************************
function eTouximenpai_NPC_wudang:CheckAccept(selfId)
	local lev = self:GetLevel(selfId)
	if lev < self.g_minLevel then
		self:NotifyTip(selfId, "你的等级太低了，根本不够我看的，还是20级之后再来找我吧。")
		return 0
...
--接受
--**********************************
function eTouximenpai_NPC_wudang:OnAccept(selfId, targetId)
...
--放弃
--**********************************
function eTouximenpai_NPC_wudang:OnAbandon(selfId)
	--不在场景的不做此操作
...
--创建副本
--**********************************
function eTouximenpai_NPC_wudang:MakeCopyScene(selfId, nearmembercount,targetId)
	if not self:LuaFnIsActivityMonster(selfId,targetId,true) then return end
	local mems = {}
	local mylevel = 0
	local i
	...
	local config                                    = {}
	config.navmapname                               = "wudang_1.nav" -- 地图是必须选取的，而且必须在Config/SceneInfo.ini里配置好
	...
	config.params[self.g_keySD["mp"]]               = define.MENPAI_ATTRIBUTE.MATTRIBUTE_WUDANG
	config.params[self.g_keySD["x"]]                = self.g_Back_X
	config.params[self.g_keySD["z"]]                = self.g_Back_Z
	config.params[13]                               = mylevel
	config.eventfile                                = "wudang_area.ini"
	config.monsterfile                              = "wudang_1_monster_" .. iniLevel .. ".ini"
...
--继续
--**********************************
function eTouximenpai_NPC_wudang:OnContinue(selfId, targetId)
...
--检测是否可以提交
--**********************************
function eTouximenpai_NPC_wudang:CheckSubmit(selfId, selectRadioId)
...
--提交
--**********************************
function eTouximenpai_NPC_wudang:OnSubmit(selfId, targetId, selectRadioId)
...
--杀死怪物或玩家
--**********************************
function eTouximenpai_NPC_wudang:OnKillObject(selfId, objdataId, objId)
	print("eTouximenpai_NPC_wudang:OnKillObject", selfId, objdataId, objId)
	--是否是副本
	...
	if (MonsterName == "喽啰") then
		...
		if killedmonsternumber == self.g_Totalkillmonstercount then
			...
			local tmpMonsterId = self:LuaFnCreateMonster(monsterLevel, 87, 51, 14, 138, -1)
...
--进入区域事件
--**********************************
function eTouximenpai_NPC_wudang:OnEnterZone(selfId, zoneId)
end

--**********************************
--道具改变
--**********************************
function eTouximenpai_NPC_wudang:OnItemChanged(selfId, itemdataId)
end

--**********************************
--副本事件
--**********************************
function eTouximenpai_NPC_wudang:OnCopySceneReady(destsceneId)
	--设置副本入口场景号
...
--有玩家进入副本事件
--**********************************
function eTouximenpai_NPC_wudang:OnPlayerEnter(selfId)
...
--有玩家在副本中死亡事件
--**********************************
function eTouximenpai_NPC_wudang:OnHumanDie(selfId, killerId)
end

--**********************************
--副本场景定时器事件
--**********************************
function eTouximenpai_NPC_wudang:OnCopySceneTimer(nowTime)
	print("eTouximenpai_NPC_wudang:OnCopySceneTimer")
	--副本时钟读取及设置
	--取得已经执行的定时次数
...
--醒目提示
--**********************************
function eTouximenpai_NPC_wudang:NotifyTip(selfId, msg)
...
--对话窗口信息提示
--**********************************
function eTouximenpai_NPC_wudang:MsgBox(selfId, targetId, msg)
	self:BeginEvent(self.script_id)
	self:AddText(msg)
	self:EndEvent()
	self:DispatchEventList(selfId, targetId)
...
return eTouximenpai_NPC_wudang
