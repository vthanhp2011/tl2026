...
local estudy_fengren = class("estudy_fengren", script_base)
estudy_fengren.script_id = 713506
estudy_fengren.g_MessageStudy = "如果你达到%d级并且肯花费#{_EXCHG%d}就可以学会缝纫技能。你决定学习么？"
estudy_fengren.g_AbilityID = define.ABILITY_FENGREN
estudy_fengren.g_AbilityName = "缝纫"
estudy_fengren.g_Name1 = "木婉清"
function estudy_fengren:OnDefaultEvent(selfId, targetId, ButtomNum, g_Npc_ScriptId, bid)
    ...
    if ButtomNum == 0 then
        local ret, demandMoney, demandExp, limitAbilityExp, limitAbilityExpShow, currentLevelAbilityExpTop, limitLevel, extraMoney, extraExp = self:LuaFnGetAbilityLevelUpConfig2(define.ABILITY_FENGREN, 1)
        if ret then
            self:BeginEvent(self.script_id)
...
function estudy_fengren:OnEnumerate(caller, selfId, targetId, bid)
    if bid then
    ...
    local ret = self:LuaFnGetAbilityLevelUpConfig(define.ABILITY_FENGREN, 1)
    if ret then
        caller:AddNumTextWithTarget(self.script_id, "学习" .. self.g_AbilityName .. "技能", 12, 0)
    end
    return
...
function estudy_fengren:CheckAccept()

end

function estudy_fengren:OnAccept()

end

function estudy_fengren:StudyInCity(selfId, targetId, ButtomNum, g_Npc_ScriptId, bid)
    if bid then
        if 0 == ButtomNum then
