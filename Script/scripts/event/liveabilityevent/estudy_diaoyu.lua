...
local estudy_diaoyu = class("estudy_diaoyu", script_base)
estudy_diaoyu.script_id = 713510
estudy_diaoyu.g_MessageStudy = "如果你达到%d级并且肯花费#{_EXCHG%d}就可以学会钓鱼技能。你决定学习么？"
estudy_diaoyu.g_AbilityID = define.ABILITY_DIAOYU
estudy_diaoyu.g_AbilityName = "钓鱼"
function estudy_diaoyu:OnDefaultEvent(selfId, targetId, ButtomNum, g_Npc_ScriptId, bid)
    ...
    if ButtomNum == 0 then
        local ret, demandMoney, demandExp, limitAbilityExp, limitAbilityExpShow, currentLevelAbilityExpTop, limitLevel, extraMoney, extraExp = self:LuaFnGetAbilityLevelUpConfig2(define.ABILITY_DIAOYU, 1)
        if ret then
            self:BeginEvent(self.script_id)
...
function estudy_diaoyu:OnEnumerate(caller, selfId, targetId, bid)
    if bid then
    ...
    local ret = self:LuaFnGetAbilityLevelUpConfig(define.ABILITY_DIAOYU, 1)
    if ret then
        caller:AddNumTextWithTarget(self.script_id, "学习" .. self.g_AbilityName .. "技能", 12, 0)
    end
    return
...
function estudy_diaoyu:CheckAccept()

end

function estudy_diaoyu:OnAccept()

end

function estudy_diaoyu:StudyInCity(selfId, targetId, ButtomNum, g_Npc_ScriptId, bid)
    if bid then
        if 0 == ButtomNum then
