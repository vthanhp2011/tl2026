...
local estudy_fofa = class("estudy_fofa", script_base)
estudy_fofa.script_id = 713529
estudy_fofa.g_MessageStudy = "如果你达到%d级并且肯花费#{_EXCHG%d}就可以学会佛法能。你决定学习么？"
estudy_fofa.g_AbilityID = define.ABILITY_FOFA
estudy_fofa.g_AbilityName = "佛法"
function estudy_fofa:OnDefaultEvent(selfId, targetId, ButtomNum, g_Npc_ScriptId)
...
function estudy_fofa:OnEnumerate(caller, selfId, targetId, bid)
    caller:AddNumTextWithTarget(self.script_id, "学习" .. self.g_AbilityName .. "技能", 12, 0)
end

function estudy_fofa:CheckAccept()

end

function estudy_fofa:OnAccept()
