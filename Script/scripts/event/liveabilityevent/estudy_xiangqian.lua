...
local estudy_xiangqian = class("estudy_xiangqian", script_base)
estudy_xiangqian.script_id = 713504
estudy_xiangqian.g_MessageStudy = "如果你达到%d级并且肯花费#{_EXCHG%d}就可以学会镶嵌技能。你决定学习么？"
estudy_xiangqian.g_AbilityID = define.ABILITY_XIANGQIAN
estudy_xiangqian.g_AbilityName = "镶嵌"
function estudy_xiangqian:OnDefaultEvent(selfId, targetId, ButtomNum, g_Npc_ScriptId, bid)
    ...
    if ButtomNum == 0 then
        local ret, demandMoney, demandExp, limitAbilityExp, limitAbilityExpShow, currentLevelAbilityExpTop, limitLevel, extraMoney, extraExp = self:LuaFnGetAbilityLevelUpConfig2(define.ABILITY_XIANGQIAN, 1)
        if ret then
            self:BeginEvent(self.script_id)
...
function estudy_xiangqian:OnEnumerate(caller, selfId, targetId, bid)
    if bid then
    ...
    local ret = self:LuaFnGetAbilityLevelUpConfig(define.ABILITY_XIANGQIAN, 1)
    if ret then
        caller:AddNumTextWithTarget(self.script_id, "学习" .. self.g_AbilityName .. "技能", 12, 0)
    end
    return
...
function estudy_xiangqian:CheckAccept()

end

function estudy_xiangqian:OnAccept()

end

function estudy_xiangqian:StudyInCity(selfId, targetId, ButtomNum, g_Npc_ScriptId, bid)
    if bid then
        if 0 == ButtomNum then
