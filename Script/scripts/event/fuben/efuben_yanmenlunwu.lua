...
efuben_yanmenlunwu.script_id = 801801
...
efuben_yanmenlunwu.g_NPCScriptID = {801802,801803,801804,801805,801806,801807,801808,801809,801810}
...
function efuben_yanmenlunwu:MakeCopyScene(selfId,nType)
    ...
    local config = {}
    config.navmapname = "Hjyanmenguan_FB.nav"
...
function efuben_yanmenlunwu:TickFubenLife()
	...
	if TickCount == 1 then
		self:LuaFnCreateMonster(49885,175,351,3,-1,-1) --创建孟闯
		self:LuaFnCreateMonster(49885,170,165,3,-1,-1) --创建孟闯
		--创建飞天坐骑
