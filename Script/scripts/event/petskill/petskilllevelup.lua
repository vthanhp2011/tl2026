...
function petskilllevelup:PetSkillLevelup(selfId, petHid, petLid, skillindex,  ItemIndexInBag)
    ...
    if checkAvailable then
        ...
        if IsBroadCast == 1 then
            ...
            local message = string.format( "#H#{_INFOUSR%s}#{J_09}#{_INFOMSG%s}#{J_10}#{_INFOMSG%s}#{J_11}#G%s#P！", gbk.fromutf8(self:LuaFnGetName(selfId)), szPetTrans, ItemInfo, gbk.fromutf8(self:GetSkillName(SkillLevelUpID)))
