...
function team:TransferFunc(selfId, newSceneId, posX, posY, minLevel, maxLevel,limit_team)
    --minLevel = nil
    --maxLevel = nil
    -- print("TransferFunc =", selfId, newSceneId, posX, posY, minLevel, maxLevel)
    if newSceneId == 2 then
        minLevel = 1
		if self:LuaFnGetDestSceneHumanCount(newSceneId) > 600 then
			self:notify_tips(selfId,"目标场景人数已满")
			return
		end
	elseif self.special_scene[newSceneId] then
		for i = #self.special_scene[newSceneId],1,-1 do
			if self:LuaFnGetDestSceneHumanCount(self.special_scene[newSceneId][i]) < 300 then
				newSceneId = self.special_scene[newSceneId][i]
				break
			end
		end
    end
