...
local ai_sangtugong = class("ai_sangtugong", script_base)

--副本逻辑脚本号....
ai_sangtugong.g_FuBenScriptId = 402263


--免疫特定技能buff....
ai_sangtugong.Buff_MianYi1	= 10472	--免疫一些负面效果....
ai_sangtugong.Buff_MianYi2	= 10471	--免疫普通隐身....

--A土遁....
ai_sangtugong.SkillA_TuDun				= 1028
ai_sangtugong.SkillA_ChildName		= "碧磷僵屍"
ai_sangtugong.SkillA_ChildBuff		= 10246
ai_sangtugong.SkillA_ChildTime		= 5000		--土遁多长时间后开始刷小怪....
ai_sangtugong.SkillA_Time					= 20000		--土遁持续的时间....


--B牛毛毒针....
ai_sangtugong.SkillB_NiuMaoDuZhen = 1029
--冷却时间....
ai_sangtugong.SkillB_CD						= 20000


--C出土文物技能的buff列表....
ai_sangtugong.SkillC_ChutuBuff1 = { 10237, 10238 }
ai_sangtugong.SkillC_ChutuBuff2 = { 10239, 10240, 10241, 10242 }


--D疯狂....
ai_sangtugong.SkillD_Buff1	= 10234
ai_sangtugong.SkillD_Buff2	= 10235
--开始进入狂暴状态的时间....
ai_sangtugong.EnterKuangBaoTime	= 5*60*1000


--AI Index....
ai_sangtugong.IDX_HPStep							= 1	--血量级别....
ai_sangtugong.IDX_SkillB_CD						= 2	--B技能的CD时间....
ai_sangtugong.IDX_KuangBaoTimer				= 3	--狂暴的计时器....
ai_sangtugong.IDX_TuDunTimer					= 4	--土遁的计时器....用于计算何时土遁结束....
ai_sangtugong.IDX_NeedCreateChildNum	= 5	--需要创建的小怪的数量....

ai_sangtugong.IDX_CombatFlag 			= 1	--是否处于战斗状态的标志....
ai_sangtugong.IDX_IsTudunMode			= 2	--是否处于土遁模式的标志....
ai_sangtugong.IDX_IsKuangBaoMode	= 3	--是否处于狂暴模式的标志....


--**********************************
--初始化....
--**********************************
function ai_sangtugong:OnInit(selfId)
	--重置AI....
...
--心跳....
--**********************************
function ai_sangtugong:OnHeartBeat(selfId, nTick)
	--检测是不是死了....
	...
	--检测是否不在战斗状态....
	if 0 == self:MonsterAI_GetBoolParamByIndex(selfId, ai_sangtugong.IDX_CombatFlag ) then
		return
	end

	--狂暴状态不需要走逻辑....
	if 1 == self:MonsterAI_GetBoolParamByIndex(selfId, ai_sangtugong.IDX_IsKuangBaoMode ) then
		return
	end
...
--进入战斗....
--**********************************
function ai_sangtugong:OnEnterCombat(selfId, enmeyId)
	--加初始buff....
	self:LuaFnSendSpecificImpactToUnit(selfId, selfId, selfId, ai_sangtugong.Buff_MianYi1, 0 )
	self:LuaFnSendSpecificImpactToUnit(selfId, selfId, selfId, ai_sangtugong.Buff_MianYi2, 0 )
	--重置AI....
	self:ResetMyAI(selfId )

	--设置进入战斗状态....
	self:MonsterAI_SetBoolParamByIndex(selfId, ai_sangtugong.IDX_CombatFlag, 1 )
end
...
--离开战斗....
--**********************************
function ai_sangtugong:OnLeaveCombat(selfId)
	-- print("ai_sangtugong:OnLeaveCombat", debug.traceback())
	--删除自己....
	self:ResetMyAI(selfId)
	self:LuaFnDeleteMonster(selfId )

	--创建对话NPC....
	local MstId = self:CallScriptFunction( ai_sangtugong.g_FuBenScriptId, "CreateBOSS", "SangTuGong_NPC", -1, -1 )
...
--杀死敌人....
--**********************************
function ai_sangtugong:OnKillCharacter(selfId, targetId)
...
--死亡....
--**********************************
function ai_sangtugong:OnDie(selfId, killerId )

	self:ResetMyAI(selfId)
	--设置已经挑战过桑土公....
	self:CallScriptFunction( ai_sangtugong.g_FuBenScriptId, "SetBossBattleFlag", "SangTuGong", 2 )

	--如果还没有挑战过乌老大则可以挑战乌老大....
	if 2 ~= self:CallScriptFunction( ai_sangtugong.g_FuBenScriptId, "GetBossBattleFlag", "WuLaoDa" )	then
		self:CallScriptFunction( ai_sangtugong.g_FuBenScriptId, "SetBossBattleFlag", "WuLaoDa", 1 )
	end
	-- zchw 全球公告
...
--重置AI....
--**********************************
function ai_sangtugong:ResetMyAI(selfId )

	--重置参数....
	self:MonsterAI_SetIntParamByIndex(selfId, ai_sangtugong.IDX_HPStep, 0 )
	self:MonsterAI_SetIntParamByIndex(selfId, ai_sangtugong.IDX_SkillB_CD, ai_sangtugong.SkillB_CD )
	self:MonsterAI_SetIntParamByIndex(selfId, ai_sangtugong.IDX_KuangBaoTimer, 0 )
	self:MonsterAI_SetIntParamByIndex(selfId, ai_sangtugong.IDX_TuDunTimer, 0 )
	self:MonsterAI_SetIntParamByIndex(selfId, ai_sangtugong.IDX_NeedCreateChildNum, 0 )

	self:MonsterAI_SetBoolParamByIndex(selfId, ai_sangtugong.IDX_CombatFlag, 0 )
	self:MonsterAI_SetBoolParamByIndex(selfId, ai_sangtugong.IDX_IsTudunMode, 0 )
	self:MonsterAI_SetBoolParamByIndex(selfId, ai_sangtugong.IDX_IsKuangBaoMode, 0 )
...
--土遁逻辑....
--**********************************
function ai_sangtugong:SkillLogicA_TunDun(selfId, nTick )
	--土遁模式则更新土遁的计时器....
	...
	--非土遁模式则检测是否可以进入土遁模式....
		...
		local LastStep = self:MonsterAI_GetIntParamByIndex(selfId, ai_sangtugong.IDX_HPStep )
		local CurStep = -1
		...
		if CurStep > LastStep then
			...
			self:MonsterAI_SetBoolParamByIndex(selfId, ai_sangtugong.IDX_IsTudunMode, 1 )
			self:MonsterAI_SetIntParamByIndex(selfId, ai_sangtugong.IDX_NeedCreateChildNum, NeedCreateNum )
			self:MonsterAI_SetIntParamByIndex(selfId, ai_sangtugong.IDX_HPStep, CurStep )
			self:MonsterAI_SetIntParamByIndex(selfId, ai_sangtugong.IDX_TuDunTimer, ai_sangtugong.SkillA_Time )
			return 1
		end
	end
	return 0
...
--牛毛毒针逻辑....
--**********************************
function ai_sangtugong:SkillLogicB_NiuMaoDuZhen(selfId, nTick )
	--更新技能CD....
...
--狂暴逻辑....
--**********************************
function ai_sangtugong:DoSkillD_KuangBao(selfId, nTick )
	--检测是否到了狂暴的时候....
