...
local QiXiFlower9999 = class("QiXiFlower9999", script_base)
QiXiFlower9999.script_id = 890079
-- QiXiFlower9999.g_levelRequire = 1
-- QiXiFlower9999.g_radiusAE = 3.0
-- QiXiFlower9999.g_standFlag = 1
-- QiXiFlower9999.g_effectCount = 4
-- QiXiFlower9999.g_Impact1 = 4918
-- QiXiFlower9999.g_Impact2 = -1
local Needitemid = 38008175
...
function QiXiFlower9999:OnDefaultEvent(selfId, bagIndex)
end

function QiXiFlower9999:IsSkillLikeScript(selfId)
    return 1
end

function QiXiFlower9999:CancelImpacts(selfId)
    return 0
end

function QiXiFlower9999:OnConditionCheck(selfId)
...
function QiXiFlower9999:OnDeplete(selfId)
    local targetId = self:LuaFnGetTargetObjID(selfId)
	local nItemId = self:LuaFnGetItemIndexOfUsedItem(selfId)
	if nItemId ~= Needitemid then
		return 0
...
function QiXiFlower9999:OnActivateOnce(selfId)
	if self:GetHumanGameFlag(selfId,"zhu_bo_flag") ~= 0 then
		self:notify_tips(selfId, "限制交易的角色不可送花。")
		return 0
	elseif self:GetHumanGameFlag(selfId,"limit_change") ~= 0 then
...
function QiXiFlower9999:OnActivateEachTick(selfId)
    return 1
end
function QiXiFlower9999:MsgBox(selfId, Msg)
    if Msg == nil then
        return
    end
    self:BeginEvent(self.script_id)
