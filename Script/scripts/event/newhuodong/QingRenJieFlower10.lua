...
local QingRenJieFlower10 = class("QingRenJieFlower10", script_base)
QingRenJieFlower10.script_id = 892967
-- QingRenJieFlower10.g_levelRequire = 1
-- QingRenJieFlower10.g_radiusAE = 3.0
-- QingRenJieFlower10.g_standFlag = 1
-- QingRenJieFlower10.g_effectCount = 4
-- QingRenJieFlower10.g_Impact1 = 4918
-- QingRenJieFlower10.g_Impact2 = -1
local Needitemid = 38002468
local FlowerCount = 10
local Additemid = 38002471
local AddCount = 10
local Needui = 1
local Haveimp = -1
local Youhadu = 4916
function QingRenJieFlower10:OnDefaultEvent(selfId, bagIndex)
end

function QingRenJieFlower10:IsSkillLikeScript(selfId)
    return 1
end

function QingRenJieFlower10:CancelImpacts(selfId)
    return 0
end

function QingRenJieFlower10:OnConditionCheck(selfId)
...
function QingRenJieFlower10:OnDeplete(selfId)
    local targetId = self:LuaFnGetTargetObjID(selfId)
	local nItemId = self:LuaFnGetItemIndexOfUsedItem(selfId)
	if nItemId ~= Needitemid then
		return 0
...
function QingRenJieFlower10:OnActivateOnce(selfId)
	if self:GetHumanGameFlag(selfId,"zhu_bo_flag") ~= 0 then
		self:notify_tips(selfId, "限制交易的角色不可送花。")
		return 0
	elseif self:GetHumanGameFlag(selfId,"limit_change") ~= 0 then
...
function QingRenJieFlower10:OnActivateEachTick(selfId)
    return 1
end
function QingRenJieFlower10:MsgBox(selfId, Msg)
    if Msg == nil then
        return
    end
    self:BeginEvent(self.script_id)
