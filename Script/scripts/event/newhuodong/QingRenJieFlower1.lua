...
local QingRenJieFlower1 = class("QingRenJieFlower1", script_base)
QingRenJieFlower1.script_id = 998523
-- QingRenJieFlower1.g_levelRequire = 1
-- QingRenJieFlower1.g_radiusAE = 3.0
-- QingRenJieFlower1.g_standFlag = 1
-- QingRenJieFlower1.g_effectCount = 4
-- QingRenJieFlower1.g_Impact1 = 4918
-- QingRenJieFlower1.g_Impact2 = -1
local Needitemid = 38002467
local FlowerCount = 1
...
local Haveimp = -1
local Youhadu = 4915
function QingRenJieFlower1:OnDefaultEvent(selfId, bagIndex)
end

function QingRenJieFlower1:IsSkillLikeScript(selfId)
    return 1
end

function QingRenJieFlower1:CancelImpacts(selfId)
    return 0
end

function QingRenJieFlower1:OnConditionCheck(selfId)
...
function QingRenJieFlower1:OnDeplete(selfId)
    local targetId = self:LuaFnGetTargetObjID(selfId)
	local nItemId = self:LuaFnGetItemIndexOfUsedItem(selfId)
	if nItemId ~= Needitemid then
		return 0
...
function QingRenJieFlower1:OnActivateOnce(selfId)
	if self:GetHumanGameFlag(selfId,"zhu_bo_flag") ~= 0 then
		self:notify_tips(selfId, "限制交易的角色不可送花。")
		return 0
	elseif self:GetHumanGameFlag(selfId,"limit_change") ~= 0 then
...
function QingRenJieFlower1:OnActivateEachTick(selfId)
    return 1
end
function QingRenJieFlower1:MsgBox(selfId, Msg)
    if Msg == nil then
        return
    end
    self:BeginEvent(self.script_id)
