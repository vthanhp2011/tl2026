...
local QingRenJieFlower999 = class("QingRenJieFlower999", script_base)
QingRenJieFlower999.script_id = 892969
-- QingRenJieFlower999.g_levelRequire = 1
-- QingRenJieFlower999.g_radiusAE = 3.0
-- QingRenJieFlower999.g_standFlag = 1
-- QingRenJieFlower999.g_effectCount = 4
-- QingRenJieFlower999.g_Impact1 = 4918
-- QingRenJieFlower999.g_Impact2 = -1
local Needitemid = 38002470
local FlowerCount = 999
...
function QingRenJieFlower999:OnDefaultEvent(selfId, bagIndex)
end

function QingRenJieFlower999:IsSkillLikeScript(selfId)
    return 1
end

function QingRenJieFlower999:CancelImpacts(selfId)
    return 0
end

function QingRenJieFlower999:OnConditionCheck(selfId)
...
function QingRenJieFlower999:OnDeplete(selfId)
    local targetId = self:LuaFnGetTargetObjID(selfId)
	local nItemId = self:LuaFnGetItemIndexOfUsedItem(selfId)
	if nItemId ~= Needitemid then
		return 0
...
function QingRenJieFlower999:OnActivateOnce(selfId)
	if self:GetHumanGameFlag(selfId,"zhu_bo_flag") ~= 0 then
		self:notify_tips(selfId, "限制交易的角色不可送花。")
		return 0
	elseif self:GetHumanGameFlag(selfId,"limit_change") ~= 0 then
...
function QingRenJieFlower999:OnActivateEachTick(selfId)
    return 1
end
function QingRenJieFlower999:MsgBox(selfId, Msg)
    if Msg == nil then
        return
    end
    self:BeginEvent(self.script_id)
