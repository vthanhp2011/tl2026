...
local QiXiFlower10 = class("QiXiFlower10", script_base)
QiXiFlower10.script_id = 890076
-- QiXiFlower10.g_levelRequire = 1
-- QiXiFlower10.g_radiusAE = 3.0
-- QiXiFlower10.g_standFlag = 1
-- QiXiFlower10.g_effectCount = 4
-- QiXiFlower10.g_Impact1 = 4918
-- QiXiFlower10.g_Impact2 = -1
local Needitemid = 38002281
...
local Haveimp = -1
local Youhadu = 4916
function QiXiFlower10:OnDefaultEvent(selfId, bagIndex)
end

function QiXiFlower10:IsSkillLikeScript(selfId)
    return 1
end

function QiXiFlower10:CancelImpacts(selfId)
    return 0
end

function QiXiFlower10:OnConditionCheck(selfId)
...
function QiXiFlower10:OnDeplete(selfId)
    local targetId = self:LuaFnGetTargetObjID(selfId)
	local nItemId = self:LuaFnGetItemIndexOfUsedItem(selfId)
	if nItemId ~= Needitemid then
		return 0
...
function QiXiFlower10:OnActivateOnce(selfId)
	if self:GetHumanGameFlag(selfId,"zhu_bo_flag") ~= 0 then
		self:notify_tips(selfId, "限制交易的角色不可送花。")
		return 0
	elseif self:GetHumanGameFlag(selfId,"limit_change") ~= 0 then
...
function QiXiFlower10:OnActivateEachTick(selfId)
    return 1
end
function QiXiFlower10:MsgBox(selfId, Msg)
    if Msg == nil then
        return
    end
    self:BeginEvent(self.script_id)
