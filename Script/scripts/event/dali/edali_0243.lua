    ...
    if index == 3 then
        ...
        if ret then
            ...
            if delret == 1 then
                ...
                if rand == 1 then
                    local fmt = gbk.fromutf8("#P突然！天昏地暗，众人皆不知所措，原来是#{_INFOUSR%s}使用#G20本#{_INFOMSG%s}#P换取到了无出其右羡煞旁人的#G %s高级门派时装#P！")
                    str = string.format(fmt, name, GemItemInfo, nMenpaiName)
                elseif rand == 2 then
                    local fmt = gbk.fromutf8("#P哇呀！#{_INFOUSR%s}使用#G20本#{_INFOMSG%s}#P换到了#G %s高级门派时装#P，穿上后真是惊人的耀眼！")
                    str = string.format(fmt, name, GemItemInfo, nMenpaiName)
                else
                    local fmt = gbk.fromutf8("#P#{_INFOUSR%s}使用#G20本#{_INFOMSG%s}#P换到了#G %s高级门派时装#P！恭喜！恭喜！再恭喜！")
                    str = string.format(fmt, name, GemItemInfo, nMenpaiName)
                end
