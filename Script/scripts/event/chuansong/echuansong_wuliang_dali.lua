...
local dunhuang = {2, 71, 72, 1320, 1321}
function chuan_song:on_enter_area(scene, obj)
    if self:LuaFnGetDestSceneHumanCount(dunhuang[1]) < 100 then
        ...
        self:CallScriptFunction((400900), "TransferFunc", obj:get_obj_id(),
                                to_scene_id, 280, 154, 1)
        return
    end
    ...
    if self:LuaFnGetDestSceneHumanCount(dunhuang[4]) < 200 then
        local to_scene_id = dunhuang[4]
        self:CallScriptFunction((400900), "TransferFunc", obj:get_obj_id(),
                                to_scene_id, 280, 154, 1)
        return
    end
    if self:LuaFnGetDestSceneHumanCount(dunhuang[5]) < 200 then
        local to_scene_id = dunhuang[5]
        self:CallScriptFunction((400900), "TransferFunc", obj:get_obj_id(),
                                to_scene_id, 280, 154, 1)
        return
    end
