...
function ejingji_2:OnEnumerate(caller, selfId, targetId, arg, index)
    caller:AddNumTextWithTarget(targetId,"盟主铜腰带兑换盟主之印", 12, 1)
    caller:AddNumTextWithTarget(targetId,"盟主银腰带兑换盟主之印", 12, 2)
    caller:AddNumTextWithTarget(targetId,"盟主金腰带兑换盟主之印", 12, 3)
    caller:AddNumTextWithTarget(targetId,"升级为盟主铜腰带", 6, 7)
    caller:AddNumTextWithTarget(targetId,"升级为盟主银腰带", 6, 4)
    caller:AddNumTextWithTarget(targetId,"升级为盟主金腰带", 6, 5)
    caller:AddNumTextWithTarget(targetId,"升级为盟主七宝腰带", 6, 6)
end
    ...
    elseif nIndex == 3 then
        ...
        if ret then
            if self:LuaFnGetAvailableItemCount(selfId, self.g_StoneId) >= 400 and
                ...
                local str = string.format("#P於九莲大喊：天下英雄们！伟大的英雄#{_INFOUSR%s}多年来在封禅台上浴血奋战，终于赢得了武林盟主真正的标志！#{_INFOMSG%s}！", name, szTransferEquip)
