...
function murongfu:OnDie(selfId, killerId)
    ...
    if ran == 1 then
        str = string.format("#W#{_INFOUSR%s}#P与#{_BOSS0}单挑，却暗使队友在其身后砸板砖、使绊子、敲闷棍、洒石灰……无所不用，终于将#{_BOSS0}打得大败，落荒而逃，一举攻下了燕子坞。",  self:GetName(nLeaderId))
    elseif ran == 2 then
        str = string.format("#W#{_INFOUSR%s}#P率领队友与#{_BOSS0}酣战半日，忽而领悟到武学的真谛，顿时武功暴涨，#{_BOSS0}抵敌不过，只得落荒而逃，燕子坞遂陷。", self:GetName(nLeaderId))
    else
        str = string.format("#W#{_INFOUSR%s}#P在燕子坞调兵遣将，运筹帷幄，在使用了瞒天过海，暗渡陈仓等三十六计之后，打得#{_BOSS0}只得使用第三十七计逃之夭夭了。",self:GetName(nLeaderId))
    end
