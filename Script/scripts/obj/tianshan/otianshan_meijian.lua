...
local otianshan_meijian = class("otianshan_meijian", script_base)
otianshan_meijian.script_id = 17001
local MenPaiPet =
{
...
function otianshan_meijian:OnDefaultEvent(selfId, targetId)
	local menpai = self:GetMenPai(selfId)
    self:BeginEvent(self.script_id)
        self:AddText("  我是梅剑，老尊主不在，我来暂时管理天山弟子。")
        if menpai == 9 then
...
function otianshan_meijian:OnEventRequest(selfId, targetId, arg, index)
	...
	if index == 0 then
		...
		if nMenPai == 7 then
			self:MsgBox(selfId,targetId,"#{event_tianshan_0001}")
			return
		end
		self:BeginEvent(self.script_id)
		self:AddText(" #{MenpaiInfo_007}")
		self:AddNumText("我确定要拜入天山派", 6,3)
		self:AddNumText("我暂时还不想拜入门派", 8,1000)
		self:EndEvent()
		self:DispatchEventList(selfId, targetId)
	elseif index == 1 then
		self:MsgBox(selfId, targetId, " #{OBJ_tianshan_0003}")
	elseif index == 6 then
		self:MsgBox(selfId, targetId, " #{MPJN_090728_05}")
	elseif index == 3 then
		...
		if self:GetMenPai(selfId) == 9 then
			self:LuaFnJoinMenpai(selfId,7)
			self:LuaFnSetXinFaLevel(selfId,43,10)
			self:LuaFnSetXinFaLevel(selfId,44,1)
			self:LuaFnSetXinFaLevel(selfId,45,1)
			self:LuaFnSetXinFaLevel(selfId,46,1)
			self:LuaFnSetXinFaLevel(selfId,47,10)
			self:LuaFnSetXinFaLevel(selfId,48,1)
			self:AddItemListToHuman(selfId)
			self:notify_tips(selfId, "你已经加入天山派！")
			self:notify_tips(selfId, "你获得了" .. self:GetItemName(10124006) .. "。")
	...
	elseif index == 100 then
		...
		self:BeginAddItem()
		self:AddItem(10434010, 1,true)
		self:AddItem(10443610, 1,true)
		local ret = self:EndAddItem(selfId)
		if not ret then
			self:MsgBox(selfId, targetId, "  你的背包空间不够了，整理后再来找我。")
			return
		...
		self:AddItemListToHuman(selfId)
		self:notify_tips(selfId, "你获得了" .. self:GetItemName(10434010) .. "。")
		self:notify_tips(selfId, "你获得了" .. self:GetItemName(10443610) .. "。")
	...
	elseif index == 102 then
		...
		if self:GetItemCount(selfId, 10124006) > 0 then
...
function otianshan_meijian:MsgBox(selfId, targetId, msg)
	self:BeginEvent(self.script_id)
	self:AddText(msg)
	self:EndEvent()
	self:DispatchEventList(selfId, targetId)
end
return otianshan_meijian