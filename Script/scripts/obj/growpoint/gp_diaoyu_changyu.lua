...
local gp_diaoyu_changyu = class("gp_diaoyu_changyu", script_base)
gp_diaoyu_changyu.g_MainItemId = 20102008
gp_diaoyu_changyu.g_AbilityId = 9
gp_diaoyu_changyu.g_AbilityLevel = 8
function gp_diaoyu_changyu:OnCreate(growPointType, x, y)
    local ItemCount = 0
    self:ItemBoxEnterScene(x, y, growPointType, define.QUALITY_MUST_BE_CHANGE, ItemCount)
end

function gp_diaoyu_changyu:OnOpen(selfId, targetId)
...
function gp_diaoyu_changyu:OnProcOver(selfId, targetId)
    local ret_1 = self:TryRecieveItem(selfId, self.g_MainItemId, define.QUALITY_MUST_BE_CHANGE)
    if ret_1 then
        self:Msg2Player(selfId, "你钓到一条鲳鱼。", define.ENUM_CHAT_TYPE.MSG2PLAYER_PARA)
        local ABilityID = self:GetItemBoxRequireAbilityID(targetId)
