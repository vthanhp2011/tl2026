...
local gp_diaoyu_baiqiyu = class("gp_diaoyu_baiqiyu", script_base)
gp_diaoyu_baiqiyu.g_MainItemId = 20102022
gp_diaoyu_baiqiyu.g_AbilityId = 9
gp_diaoyu_baiqiyu.g_AbilityLevel = 10
function gp_diaoyu_baiqiyu:OnCreate(growPointType, x, y)
    local ItemCount = 0
    self:ItemBoxEnterScene(x, y, growPointType, define.QUALITY_MUST_BE_CHANGE, ItemCount)
end

function gp_diaoyu_baiqiyu:OnOpen(selfId, targetId)
...
function gp_diaoyu_baiqiyu:OnProcOver(selfId, targetId)
    local ret_1 = self:TryRecieveItem(selfId, self.g_MainItemId, define.QUALITY_MUST_BE_CHANGE)
    if ret_1 then
        self:Msg2Player(selfId, "你钓到一条摆鳍鱼。", define.ENUM_CHAT_TYPE.MSG2PLAYER_PARA)
        local ABilityID = self:GetItemBoxRequireAbilityID(targetId)
