...
local gp_treasuredropper_bingkang = class("gp_treasuredropper_bingkang", script_base)
gp_treasuredropper_bingkang.g_DropNumTable = {
    {["num"] = 1, ["odd"] = 0.2},
    {["num"] = 2, ["odd"] = 0.4},
    {["num"] = 3, ["odd"] = 0.2},
    {["num"] = 4, ["odd"] = 0.15},
...
gp_treasuredropper_bingkang.g_TickCreate_Msg = ""
gp_treasuredropper_bingkang.g_DropTable = {
    {
        ["itemType"] = 1,
        ["odd"] = 0.285,
        ["numOdd"] = {
    ...
    },
    {
        ...
        ["idx"] = {30301151, 30301152, 30301153, 30301154, 30301155, 30301156, 30301157, 30301158, 30301159}
    },
    {
        ["itemType"] = 9,
        ["odd"] = -1.0,
...
function gp_treasuredropper_bingkang:GetTableIndexByOdd(tb)
...
function gp_treasuredropper_bingkang:OnCreate(growPointType, x, y, dur)
    local ItemBoxId = nil
        ...
        for i = 1, self.g_DropNumTable[numIdx]["num"] do
            ...
            if (showIdx) then
                ...
                elseif (dropNumIdx and 9 == self.g_DropTable[showIdx]["itemType"]) then
                    ...
                    for k = 1, self.g_DropTable[showIdx]["numOdd"][dropNumIdx]["num"] do
                        ...
                        print(
                            sceneId .. " 号场景 (" .. x .. ", " .. y .. ") 处长出一只珍兽 " .. self:GetName(PetObjId) .. "。"
                        )
                    end
                end
            end
            if (showIdx and dropNumIdx and itemId ~= nil) then
                if (9 ~= self.g_DropTable[showIdx]["itemType"]) then
                    local itemName = self:GetItemInfoByItemId(itemId)
                    print(
                        "物品箱里有(类型" ..
...
function gp_treasuredropper_bingkang:OnTickCreateFinish(growPointType, tickCount)
    local sceneId = self:get_scene():get_id()
...
function gp_treasuredropper_bingkang:OnOpen(selfId, targetId)
end

function gp_treasuredropper_bingkang:OnRecycle(selfId, targetId)
    return 1
end

function gp_treasuredropper_bingkang:OnProcOver(selfId, targetId)
end

function gp_treasuredropper_bingkang:OpenCheck(selfId, AbilityId, AblityLevel)
    return define.OPERATE_RESULT.OR_OK
end

return gp_treasuredropper_bingkang
