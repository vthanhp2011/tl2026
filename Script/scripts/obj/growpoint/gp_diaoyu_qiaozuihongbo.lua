...
local gp_diaoyu_qiaozuihongbo = class("gp_diaoyu_qiaozuihongbo", script_base)
gp_diaoyu_qiaozuihongbo.g_MainItemId = 20102044
gp_diaoyu_qiaozuihongbo.g_AbilityId = 9
gp_diaoyu_qiaozuihongbo.g_AbilityLevel = 9
function gp_diaoyu_qiaozuihongbo:OnCreate(growPointType, x, y)
    local ItemCount = 0
    self:ItemBoxEnterScene(x, y, growPointType, define.QUALITY_MUST_BE_CHANGE, ItemCount)
end

function gp_diaoyu_qiaozuihongbo:OnOpen(selfId, targetId)
...
function gp_diaoyu_qiaozuihongbo:OnProcOver(selfId, targetId)
    local ret_1 = self:TryRecieveItem(selfId, self.g_MainItemId, define.QUALITY_MUST_BE_CHANGE)
    if ret_1 then
        self:Msg2Player(selfId, "你钓到一条翘嘴红鲌。", define.ENUM_CHAT_TYPE.MSG2PLAYER_PARA)
        local ABilityID = self:GetItemBoxRequireAbilityID(targetId)
...
return gp_diaoyu_qiaozuihongbo
