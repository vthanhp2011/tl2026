...
function odali_wuyazi:AskXiuLianLevelUp(selfId, targetId, mjId)
	mjId = mjId + 1
	local xiulian_level = self:GetXiuLianLevel(selfId, mjId)
	xiulian_level = xiulian_level + 1
	local xiulian_detail = self:GetXiuLianDetail(selfId, mjId, xiulian_level)
	if not xiulian_detail then
		self:notify_tips(selfId, "查表异常:id = "..tostring(mjId)..",lv = "..tostring(xiulian_level))
		return
	end
	...
	local cost_gongli = self:GetCostGongLi(xiulian_level)
	local nGongLi = self:GetGongLi(selfId)
	if nGongLi < cost_gongli then
		self:notify_tips(selfId, "功力不足")
		return
	end
	local can = self:XiuLianCanLevelUp(selfId, mjId)
	if not can then
	...
	self:XiuLianLevelUp(selfId, mjId)
...
function odali_wuyazi:AskXiuLianAdvanceJingJie(selfId, targetId, mjId)
	...
	local CurMaxLevel = self:GetXiuLianLevelUpperLimit(selfId, mjId)
	...
	self:LuaFnCostMoneyWithPriority(selfId, cost_money)
	self:XiuLianLevelUpperLimitUp(selfId, mjId, 10)
	self:LuaFnSendSpecificImpactToUnit(selfId, selfId, selfId, 49, 0)
end
