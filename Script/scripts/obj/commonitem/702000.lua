...
function common_item:OnActivateOnce(selfId)
	...
	if not petGUID_H or petGUID_H == -1 then
		 self:notify_tips( selfId, "目标珍兽异常")
		return 0
	end
	if not petGUID_L or petGUID_L == -1 then
		 self:notify_tips( selfId, "目标珍兽异常")
		return 0
	end
	...
	-- 消耗珍兽滋补丹完毕
	if g_HPValue > 0 then
		self:CallScriptFunction(g_petCommonId, "IncPetHP", selfId, g_HPValue )
	end
