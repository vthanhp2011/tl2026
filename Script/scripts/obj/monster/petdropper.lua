...
petdropper.g_PetDropList[3823] = {
    ["dropOdds"] = 5000,
    ["petList"] = {6680, 6689, 6681, 6682, 6683},
    ["oddsList"] = {100000, 25088, 1254}
}

petdropper.g_PetDropList[3824] = {
    ["dropOdds"] = 5000,
    ["petList"] = {6690, 6699, 6691, 6692, 6693, 6694},
    ["oddsList"] = {100000, 25088, 1254}
}

petdropper.g_PetDropList[3825] = {
    ["dropOdds"] = 5000,
    ["petList"] = {6700, 6709, 6701, 6702, 6703, 6704, 6705},
    ["oddsList"] = {100000, 25088, 1254}
}

petdropper.g_PetDropList[3826] = {
    ["dropOdds"] = 5000,
    ["petList"] = {6710, 6719, 6711, 6712, 6713, 6714, 6715, 6716},
    ["oddsList"] = {100000, 25088, 1254}
}

petdropper.g_PetDropList[3827] = {
    ["dropOdds"] = 5000,
    ["petList"] = {6710, 6719, 6711, 6712, 6713, 6714, 6715, 6716},
    ["oddsList"] = {100000, 25088, 1254}
}

petdropper.g_PetDropList[3828] = {
    ["dropOdds"] = 5000,
    ["petList"] = {6710, 6719, 6711, 6712, 6713, 6714, 6715, 6716},
    ["oddsList"] = {100000, 25088, 1254}
}

petdropper.g_PetDropList[3829] = {
    ["dropOdds"] = 5000,
    ["petList"] = {6710, 6719, 6711, 6712, 6713, 6714, 6715, 6716},
    ["oddsList"] = {100000, 25088, 1254}
}

petdropper.g_PetDropList[11353] = {
    ["dropOdds"] = 10000,
    ["petList"] = {3310, 3310, 3310},
    ["oddsList"] = {100000, 49706, 2485}
}

petdropper.g_PetDropList[13280] = {
    ["dropOdds"] = 225,
    ["petList"] = {8180, 8189, 8181, 8182, 8183, 8184, 8185, 8186},
    ["oddsList"] = {100000, 10000, 2500}
}

petdropper.g_PetDropList[13281] = {
    ["dropOdds"] = 225,
    ["petList"] = {3290, 3299, 3291, 3292, 3293, 3294, 3295, 3296},
    ["oddsList"] = {100000, 10000, 2500}
}

petdropper.g_PetDropList[8630] = {
    ["dropOdds"] = 225,
    ["petList"] = {8630, 8631, 8632, 8633, 8634, 8639},
    ["oddsList"] = {100000, 10000, 2500}
}

petdropper.g_PetDropList[8640] = {
    ["dropOdds"] = 225,
    ["petList"] = {8640, 8641, 8642, 8643,8644, 8649},
    ["oddsList"] = {100000, 10000, 2500}
}

petdropper.g_PetDropList[8650] = {
    ["dropOdds"] = 225,
    ["petList"] = {8650, 8651, 8652, 8653,8654, 8659},
    ["oddsList"] = {100000, 10000, 2500}
}

petdropper.g_PetDropList[8660] = {
    ["dropOdds"] = 225,
    ["petList"] = {8660, 8661, 8662, 8663,8664, 8669},
    ["oddsList"] = {100000, 10000, 2500}
}


function petdropper:OnDie(objId, killerId)
    -- print("petdropper:OnDie =", objId, killerId)
	...
	if sceneId == 394 then
		if obj:get_scene_params(define.MONSTER_DATAID) == 11353 then
	...
	-- if self:LuaFnGetNpcIntParameter(objId,2) ~= sceneId then
			-- self:notify_tips(killerId, "非活动创建")
		-- return
	-- end
	-- local actId = self:LuaFnGetNpcIntParameter(objId,3)
	...
	-- if not self:CheckActiviyValidity(actId) then
		-- self:notify_tips(killerId, "活动已结束")
		-- return
	-- end
	
	
    local petDropInfo = self.g_PetDropList[self:GetMonsterDataID(objId)]
    if not petDropInfo then
        return
    end
    local rand = math.random(self.g_RandNum)
    if rand > petDropInfo["dropOdds"] then
        return
    end
    local oddsList = {}

    for index, value in pairs(petDropInfo["oddsList"]) do
        oddsList[index] = value
    end
    local petType = 1
    if petDropInfo["petTypeOdds"] then
        local typeCount = #(petDropInfo["petTypeOdds"])
        rand = 0
    ...
    local posX, posZ = self:LuaFnGetWorldPos(objId)
    if not posX or not posZ or posX < 0 or posZ < 0 then
        return
    end
    local petObjID = self:CreatePetOnScene(petDataID, posX, posZ)
    if not petObjID or petObjID < 0 then
        return
    end
    local occupantGuid = self:LuaFnGetOccupantGUID(objId)
    if not occupantGuid or occupantGuid < 0 then
        if killerId and killerId > -1 then
            local nObjType = self:GetCharacterType(killerId)
            if nObjType then
                local nHumanId = -1
                if nObjType == "human" then
                    nHumanId = killerId
                elseif nObjType == "pet" then
                    nHumanId = self:GetPetCreator(killerId)
                end
                if nHumanId and nHumanId ~= -1 then
                    occupantGuid = self:LuaFnGetGUID(nHumanId)
                end
            end
        end
    end
    ...
    self:SetPetCaptureProtect(petObjID, occupantGuid)
    self:SetCharacterDieTime(petObjID, self.g_Lifecycle)
    self:LuaFnAuditPetCreate(occupantGuid, petDataID)
end
